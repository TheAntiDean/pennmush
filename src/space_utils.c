...
int
db2sdb(dbref name) /* Returns the sdb# of a DB# space object */
{
  register int i;
...
double
xyz2elevation(double x, double y, double z)
{
...
double
xyz2range(double xa, double ya, double za, double xb, double yb, double zb)
{
  double x = xb - xa;
  double y = yb - ya;
  double z = zb - za;
...
double
xyz2vis(double x, double y, double z)
{
  double px = x / PARSEC;
  double py = y / PARSEC;
  double pz = z / PARSEC;
...
sdb2arc(int n1, int n2)
{
  ...
  if (r == 0.0) {
    ...
    v1 = (x * sdb[n1].course.d[0][0] + y * sdb[n1].course.d[0][1] +
          z * sdb[n1].course.d[0][2]) /
         r /
         sqrt(sdb[n1].course.d[0][0] * sdb[n1].course.d[0][0] +
              sdb[n1].course.d[0][1] * sdb[n1].course.d[0][1] +
              sdb[n1].course.d[0][2] * sdb[n1].course.d[0][2]);
    v2 = (x * sdb[n1].course.d[1][0] + y * sdb[n1].course.d[1][1] +
          z * sdb[n1].course.d[1][2]) /
         r /
         sqrt(sdb[n1].course.d[1][0] * sdb[n1].course.d[1][0] +
              sdb[n1].course.d[1][1] * sdb[n1].course.d[1][1] +
              sdb[n1].course.d[1][2] * sdb[n1].course.d[1][2]);
    v3 = (x * sdb[n1].course.d[2][0] + y * sdb[n1].course.d[2][1] +
          z * sdb[n1].course.d[2][2]) /
         r /
         sqrt(sdb[n1].course.d[2][0] * sdb[n1].course.d[2][0] +
              sdb[n1].course.d[2][1] * sdb[n1].course.d[2][1] +
              sdb[n1].course.d[2][2] * sdb[n1].course.d[2][2]);
...
int
arc_check(int contact, int weapon)
{
...
int
get_empty_sdb() /* Returns empty sdb slot, or VACANCY_FAIL if there is none */
{
  register int i;
...
int
contact2sdb(int x, int c) /* Returns the sdb# of a contact# */
{
  register int i;
...
int
sdb2contact(int x, int s) /* Returns the contact# of an sdb# */
{
  register int i;
...
int
contact2slist(int x, int c) /* Returns the slist# of a contact# */
{
  register int i;
...
int
sdb2slist(int x, int s) /* Returns the slist# of an sdb# */
{
  register int i;
...
int
name2sdb(char *name) /* Returns the sdb# of a named space object */
{
  register int i;
...
sdb2max_warp(int x)
{
  ...
  double p = (0.99 * sdb[x].power.main) +
             (0.01 * sdb[x].power.total * sdb[x].alloc.movement);
  ...
  if (sdb[x].status.tractoring) {
    a *= (sdb[x].structure.displacement +
          sdb[sdb[x].status.tractoring].structure.displacement + 0.1) /
         (sdb[x].structure.displacement + 0.1);
  } else if (sdb[x].status.tractored)
    a *= (sdb[x].structure.displacement +
          sdb[sdb[x].status.tractored].structure.displacement + 0.1) /
         (sdb[x].structure.displacement + 0.1);
...
double
sdb2max_impulse(int x)
{
  double a = sdb[x].move.ratio;
  double p = (0.9 * sdb[x].power.aux) +
             (0.1 * sdb[x].power.total * sdb[x].alloc.movement);
...
sdb2ecm_lrs(int x)
{
  if (sdb[x].sensor.ew_active) {
    return sqrt(1.0 + sdb[x].power.total * sdb[x].alloc.ecm *
                        sdb[x].sensor.ew_damage * sdb[x].tech.sensors / 10.0);
  } else
    return 1.0;
}
...
{
  if (sdb[x].sensor.ew_active) {
    return sqrt(1.0 + sdb[x].power.total * sdb[x].alloc.eccm *
                        sdb[x].sensor.ew_damage * sdb[x].tech.sensors / 10.0);
  } else
    return 1.0;
}
...
sdb2ecm_srs(int x)
{
  if (sdb[x].sensor.ew_active) {
    return sqrt(1.0 + sdb[x].power.total * sdb[x].alloc.ecm *
                        sdb[x].sensor.ew_damage * sdb[x].tech.sensors);
  } else
    return 1.0;
}
...
double
sdb2dissipation(int x, int shield)
{
  double d;

  if (sdb[x].shield.active[shield] && sdb[x].shield.damage[shield] > 0.0) {
    d = (2 - pow(2, (1 - sdb[x].alloc.shield[shield] * sdb[x].power.total *
                           sdb[x].shield.damage[shield] * sdb[x].shield.ratio *
                           sdb[x].sensor.visibility / sdb[x].shield.maximum))) *
...
double
xyz2cochranes(double x, double y, double z)
{
  double px = x / PARSEC;
  double py = y / PARSEC;
  double pz = z / PARSEC;
...
sdb2angular(int n1, int n2)
{
  ...
  b[0] = (sdb[n2].move.v * sdb[n2].course.d[0][0]) -
         (sdb[n1].move.v * sdb[n1].course.d[0][0]) + a[0];
  b[1] = (sdb[n2].move.v * sdb[n2].course.d[0][1]) -
         (sdb[n1].move.v * sdb[n1].course.d[0][1]) + a[1];
  b[2] = (sdb[n2].move.v * sdb[n2].course.d[0][2]) -
         (sdb[n1].move.v * sdb[n1].course.d[0][2]) + a[2];

  ...
  if (!Wizard(Owner(executor))) {
      ...
      for (i = MIN_COMMS_OBJECTS; i <= MAX_COMMS_OBJECTS; ++i)
        if (cdb[i].object) {
          if (!GoodObject(cdb[i].object) || !Hasprivs(Owner(cdb[i].object))) {
            notify(executor,
                   tprintf("#%d/%d: bad object deleted.", cdb[i].object, i));
            cdb[i].object = 0;
            cdb[i].lower = 0.0;
            cdb[i].upper = 0.0;
          } else {
            ...
            if (cdb[i].lower < MIN_COMMS_FREQUENCY) {
              notify(executor, tprintf("#%d/%d: bad lower bandpass fixed.",
                                       cdb[i].object, i));
              cdb[i].lower = MIN_COMMS_FREQUENCY;
            }
            if (cdb[i].upper > MAX_COMMS_FREQUENCY) {
              notify(executor, tprintf("#%d/%d: bad upper bandpass fixed.",
                                       cdb[i].object, i));
              cdb[i].upper = MAX_COMMS_FREQUENCY;
            }
            if (cdb[i].upper < cdb[i].lower) {
              notify(executor,
                     tprintf("#%d/%d: bad bandpass fixed.", cdb[i].object, i));
    ...
    case 'd': /* delete from CDB <void> */
      for (i = MIN_COMMS_OBJECTS; i <= max_comms_objects; ++i)
        if (cdb[i].object == executor) {
          notify(executor,
                 tprintf("#%d/%d: object deleted.", cdb[i].object, i));
          cdb[i].object = 0;
          cdb[i].lower = 0.0;
          cdb[i].upper = 0.0;
          safe_chr('1', buff, bp);
    ...
    case 's': /* set bandpass <lower> <upper> */
      for (i = MIN_COMMS_OBJECTS; i <= max_comms_objects; ++i)
        if (cdb[i].object == executor) {
          ...
          notify(executor,
                 tprintf("#%d/%d: bandpass set to %f (lower) and %f (upper).",

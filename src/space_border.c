...
free_borderinfo(void *ptr)
{
  ...
  mush_free((char*)border->name, "spaceborder_name");
  mush_free(border, "border_info");
}

void
addNewBorder(dbref executor, int border_number, const char *name, int space,
             double radius, double x, double y, double z, char *buff, char **bp)
{
  ...
  if (im_insert(border_map, border_number, newBorder)) {
    ...
    write_spacelog(executor, executor,
                   tprintf("Border added: %s", newBorder->name));
    int i = 0;
    ...
    mush_free((char*)newBorder->name, "spaceborder_name");
    safe_str("#-1 BORDER NOT CREATED.", buff, bp);
  }
}
void
deleteBorder(dbref executor, int border, char *buff, char **bp)
{
  ...
  if (theBorder != NULL) {
    write_spacelog(executor, executor,
                   tprintf("Border deleted: %s", theBorder->name));
...
char *
border_line_bot(int border_id, space_border *sbi)
{
  static char buffer[1000];

  if (sbi != NULL) {
    snprintf(buffer, sizeof(buffer), "%d|%s|%d|%.1f|%.1f|%.1f|%.1f", border_id,
             sbi->name, sbi->space, sbi->radius, sbi->x, sbi->y, sbi->z);
  }
...
void
list_borders(char *buff, char **bp)
{
  int first = 0;
  int index = 0;
...
void
edit_border(dbref executor, int border_id, const char *setting,
            const char *new_value, char *buff, char **bp)
{
  ...
  if (si != NULL) {
    switch (setting[0]) {
    case 'n': /* Name */
      mush_free((char*)si->name, "spaceborder_name");
      si->name = mush_strdup(new_value, "spaceborder_name");
      safe_format(buff, bp, "Border Name changed for border %u to %s",
                  border_id, si->name);
      write_spacelog(executor, executor,
                     tprintf("Border modified: %s", si->name));
      break;
    case 'r': /* Radius */
      si->radius = parse_number(new_value);
      safe_format(buff, bp, "Border Radius changed for border %u to %f",
                  border_id, si->radius);
      write_spacelog(executor, executor,
                     tprintf("Border modified: %s", si->name));
      break;
    case 's': /* Space */
      ...
      safe_format(buff, bp, "Border Space changed for border %u to %d",
                  border_id, si->space);
      write_spacelog(executor, executor,
                     tprintf("Border modified: %s", si->name));
      break;
    case 'x': /* X Coordinate of Centre */
      ...
      safe_format(buff, bp, "Border CoordX changed for border %u to %f",
                  border_id, si->x);
      write_spacelog(executor, executor,
                     tprintf("Border modified: %s", si->name));
      break;
    case 'y': /* Y Coordinate of Centre */
      ...
      safe_format(buff, bp, "Border CoordY changed for border %u to %f",
                  border_id, si->y);
      write_spacelog(executor, executor,
                     tprintf("Border modified: %s", si->name));
      break;
    case 'z': /* Z Coordinate of Centre */
      ...
      safe_format(buff, bp, "Border CoordZ changed for border %u to %f",
                  border_id, si->z);
      write_spacelog(executor, executor,
                     tprintf("Border modified: %s", si->name));
      break;
    default:
      safe_str("#-1 BORDER SETTING NOT FOUND.", buff, bp);
      break;
    }
  } else {
    safe_str("#-1 BORDER NOT FOUND.", buff, bp);
...
void
alert_enter_empire(int x)
{
  console_message(x, "helm",
...
/*
 * Alerts any space-object that is in range of the inbound or outbound border
 * crossing
 */
void
alert_border_cross(int x, int way)
{
  register int i;

  // Only alert if we're actually moving. Otherwise this is a fluke of the
  // dynamic borders, or Admin placing objects.
          ...
          if (sdb[i].move.empire == sdb[x].move.empire)
            if (i != x)
              if (sdb2range(x, i) < MAX_NOTIFICATION_DISTANCE) {
                if (way) {
                  console_message(
                    i, "helm science security",
                    ansi_warn(tprintf(
                      "Inbound border crossing reported at %.3f %.3f %.3f",
                  ...
                  console_message(
                    i, "helm science security",
                    ansi_warn(tprintf(
                      "Outbound border crossing reported at %.3f %.3f %.3f",
                      su2pc(sdb[x].coords.x - sdb[i].coords.xo),
                      su2pc(sdb[x].coords.y - sdb[i].coords.yo),
                      su2pc(sdb[x].coords.z - sdb[i].coords.zo))));
              }
  ...
  for (i = 1; i <= im_count(border_map); ++i) {
    ...
    if (border) {
      ...
      if (range <= (border->radius * border->radius)) { // In radius
        if (best_empire <= 0 ||
            inside_range <
              best_range) { // Closer to the center of this than previous best
          best_range = range;
          best_empire = i;
        }
      }
  ...
  if (sdb[x].move.empire != best_empire) {
    if (sdb[x].move.empire != 0) {
      ...
      if (ptr && optr && !strcasecmp(ptr->name, optr->name))
        same = 1;
      if (ptr && !same) { // Exit silently if old border doesnt exist, or if the
                          // name matches the new border
        alert_exit_empire(x);
        if ((int) get_random_u32(1, 100) <
            ((int) (sdb[x].sensor.lrs_signature * sdb[x].sensor.visibility *
                    100.0))) {
 ...
 */
int
do_border_report(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    snprintf(buffer, sizeof(buffer),
             "%s%s--[%sBorder "
             "Report%s]--------------------------------------------------------"
             "------%s\n%sEmpire               Bearing Range   Status   Center "
             "Coordinates%s\n%s-------------------- ------- ------- -------- "
             "---------------------------------%s\n",
    ...
    for (index = 1; index <= im_count(border_map); index++) {
      ...
      if (sbi != NULL) {
        if (sbi->space != sdb[n].space)
          continue;
        range = xyz2range(sdb[n].coords.x, sdb[n].coords.y, sdb[n].coords.z,
                          sbi->x, sbi->y, sbi->z) /
                PARSEC;
        ...
        if (range > sbi->radius) {
          strncat(buffer,
                  tprintf("%3d %-3d %-7.7s Outside  ",
                          (int) xy2bearing((sbi->x - sdb[n].coords.x),
                                           (sbi->y - sdb[n].coords.y)),
                          (int) xyz2elevation((sbi->x - sdb[n].coords.x),
                                              (sbi->y - sdb[n].coords.y),
                                              (sbi->z - sdb[n].coords.z)),
                          unparse_range((range - sbi->radius) * PARSEC)),
                  sizeof(buffer) - 1);
        } else {
          strncat(buffer,
                  tprintf("%3d %-3d %-7.7s Inside   ",
                          (int) xy2bearing((sdb[n].coords.x - sbi->x),
                                           (sdb[n].coords.y - sbi->y)),
                          (int) xyz2elevation((sdb[n].coords.x - sbi->x),
                                              (sdb[n].coords.y - sbi->y),
                                              (sdb[n].coords.z - sbi->z)),
                          unparse_range((sbi->radius - range) * PARSEC)),
                  sizeof(buffer) - 1);
        }
        strncat(buffer,
                tprintf("%10.3f %10.3f %10.3f\n",
                        (sbi->x - sdb[n].coords.xo) / PARSEC,
                        (sbi->y - sdb[n].coords.yo) / PARSEC,
                        (sbi->z - sdb[n].coords.zo) / PARSEC),
                sizeof(buffer) - 1);
      }
    }

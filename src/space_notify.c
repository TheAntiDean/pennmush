...
void
do_console_notify(int x, dbref parent1, dbref parent2, dbref parent3,
                  const char *msg)
{
  ATTR *a, *b;
  ...
  if (a != NULL) {
    ...
    while (pq) {
      ...
      if (GoodObject(console)) {
        ...
        if ((parent == parent1) || (parent == parent2) || (parent == parent3) ||
            (parent == console_monitor) || (parent == console_fighter)) {
...
void
do_all_console_notify(int x, const char *msg)
{
  ATTR *a, *b;
  dbref console, user;
  char *q, *pq;
...
void
do_ship_notify(int x, const char *msg)
{
  ATTR *a;
  dbref room;
  char *q, *pq;
...
void
do_space_notify_one(int x, dbref parent1, dbref parent2, dbref parent3,
                    const char *msg)
{
  ATTR *a, *b;
                          ...
                          if (b != NULL) {
                            ...
                            if (GoodObject(user)) {
                              snprintf(buffer, sizeof(buffer),
                                       "%s[%s%s %s%s]%s", ANSI_BLUE, ANSI_CYAN,
                                       unparse_identity(i, x), msg, ANSI_BLUE,
                                       ANSI_WHITE);
                              notify(user, buffer);
                            }
                          }
...
void
do_space_notify_two(int n1, int n2, dbref parent1, dbref parent2, dbref parent3,
                    const char *msg)
{
  ATTR *a, *b;
  static char buffer[500];
                          ...
                          if (b != NULL) {
                            ...
                            if (GoodObject(user)) {
                              snprintf(buffer, sizeof(buffer), "%s[%s%s %s ",
                                       ANSI_BLUE, ANSI_CYAN,
                                       unparse_identity(i, n1), msg);
                              strncat(buffer,
                                      tprintf("%s%s]%s",
                                              unparse_identity(i, n2),

...
int
do_set_fire(int first, int last, int weapon, int mode, dbref enactor)
{
  static char buff_n[1000], buff_x[MAX_SENSOR_CONTACTS][1000];
  int a, b, i, j, k, x = 0, flag = 0;
  int is_b_active = 0, is_b_lock = 0, is_b_load = 0, is_b_arm = 0, is_b_arc = 0,
      is_b_range = 0;
  int is_m_active = 0, is_m_lock = 0, is_m_load = 0, is_m_arm = 0, is_m_arc = 0,
      is_m_range = 0;
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s has no %ss.", Name(sdb[n].object),
                                     system_name[3])));
    return 0;
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no weapons.", Name(sdb[n].object))));
    return 0;
    ...
    notify(enactor,
           ansi_red(tprintf("%s is on a landing pad.", Name(sdb[n].object))));
    return 0;
  } else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
    notify(enactor, ansi_red(tprintf("%s cannot fire weapons while cloaked.",
                                     Name(sdb[n].object))));
    return 0;
  }
  ...
  if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
    ...
    for (i = a; i <= b; ++i) {
      ...
      prob /= 1.0 + (sdb2angular(n, x) * 10.0 * (1.0 + sdb[x].move.ratio) /
                     sdb[x].move.ratio);
      ...
      if (get_random_long(0, 101) < prob * 100.0) {
        ...
        if (sdb[x].tmp.d0 > range &&
            sdb[n].blist.name[i] != 19) // Mass Driver Kludge - Jarek
          dmg_b[i] *= (1.0 - (sdb[x].tmp.d0 - range) / (18.0 * range));
        strncat(buff_x[sdb[x].tmp.i1],
                tprintf("B%d:%s%s%d%s%s", i + 1, ANSI_HILITE, ANSI_RED,
                        (int) (dmg_b[i] + 0.5), ANSI_NORMAL, ANSI_CYAN),
                sizeof(buff_x[sdb[x].tmp.i1]) - 1);
        if (!sdb[x].tmp.i5)
          ++sdb[x].tmp.i5;
      } else {
        strncat(buff_x[sdb[x].tmp.i1], tprintf("B%d:--", i + 1),
                sizeof(buff_x[sdb[x].tmp.i1]) - 1);
      }
    }
  }
  ...
  if (weapon == 2 || (!weapon && sdb[n].missile.exist)) {
    ...
    for (i = a; i <= b; ++i) {
      ...
      if (sdb[x].tmp.d0 == 0.0)
        sdb[x].tmp.d0 = sdb2range(n, x);
      if ((fabs(sdb[n].move.out) >= 1.0) ||
          ((sdb[n].structure.type > 1) && (fabs(sdb[x].move.out) >= 1.0))) {
      ...
      if (get_random_long(0, 101) < prob * 100.0) {
        ...
        strncat(buff_x[sdb[x].tmp.i1],
                tprintf("M%d:%s%s%d%s%s", i + 1, ANSI_HILITE, ANSI_RED,
                        (int) (dmg_m[i] + 0.5), ANSI_NORMAL, ANSI_CYAN),
                sizeof(buff_x[sdb[x].tmp.i1]) - 1);
        if (!sdb[x].tmp.i5)
          ++sdb[x].tmp.i5;
      } else {
        strncat(buff_x[sdb[x].tmp.i1], tprintf("M%d:--", i + 1),
                sizeof(buff_x[sdb[x].tmp.i1]) - 1);
      }
    }
  }
  ...
  if (!weapon && !is_b_load && !is_m_load) {
    if (!is_b_active) {
      ...
      notify(enactor, ansi_red(tprintf("No %ss have targets in firing arc.",
                                       system_name[3])));
    } else if (!is_b_range) {
      notify(enactor, ansi_red(tprintf("No %ss have targets in range.",
                                       system_name[3])));
    } else if (!is_b_load) {
      notify(enactor,
             ansi_red(tprintf("No %ss are recycled.", system_name[3])));
    }
    if (!is_m_active) {
      ...
      notify(enactor,
             ansi_red(tprintf("No %ss are recycled.", system_name[9])));
    }
    for (i = 0; i < sdb[n].sensor.contacts; ++i) {
      x = sdb[n].slist.sdb[i];
      if (sdb[x].tmp.i0 == n) {
  ...
  for (i = 0; i < sdb[n].sensor.contacts; ++i) {
    ...
    if (sdb[x].tmp.i0 == n) {
      if (sdb[x].tmp.i4) {
        ...
        ++flag;
        strncat(buff_n,
                ansi_cmd(enactor, tprintf("Firing at %s: %s",
                                          unparse_identity(n, x), buff_x[i])),
                sizeof(buff_n) - 1);
        if (sdb[x].tmp.i5) {
          do_space_notify_two(n, x, console_helm, console_tactical,
                              console_science, "fires and hits");
          write_spacelog(
            enactor, sdb[n].object,
            tprintf("LOG: Fired and hit, Beam %.6f GHz, Missile %.6f GHz",
                    sdb[n].beam.freq, sdb[n].missile.freq));
        } else {
          do_space_notify_two(n, x, console_helm, console_tactical,
                              console_science, "fires and misses");
        }
        if (sdb[x].status.active && !sdb[x].status.crippled) {
          console_message(
            x, "helm science tactical",
            ansi_warn(
              tprintf("%s firing: %s", unparse_identity(x, n), buff_x[i])));
  ...
  if (sdb[x].tmp.d0 > 0.0 &&
      sdb[x].tmp.d2 > 0.0) /* This if statement added by Kaly */
  {
      ...
      if (sdb[x].tmp.d2 > 0.0) {
        ...
        for (j = 0; j < k; ++j) {
          ...
          if (mode == 0) /* normal damage */ {
            ...
            case 2:
              d_shield[get_random_long(0, MAX_SHIELD_NAME)] += pdmg;
              break;
            case 3:
              d_system[11] += pdmg;
              break;
            ...
            case 7:
              ...
              break;
            case 8:
              d_missile[get_random_long(0, sdb[x].missile.tubes)] += pdmg;
              break;
            case 14:
              d_beam[get_random_long(0, sdb[x].beam.banks)] += pdmg;
              break;
            case 15:
              d_system[14] += pdmg;
              break;
          ...
          if (sdb[x].structure.superstructure <= -ss) {
            atr_add(sdb[x].object, "COMMENT",
                    tprintf("%s(#%d) killed by %s(#%d) on %s(#%d) at %s",
                            Name(sdb[x].object), sdb[x].object, Name(enactor),
                            enactor, Name(sdb[n].object), sdb[n].object,
                            (char *) ctime(&mudtime)),
                    GOD, (AF_MDARK + AF_WIZARD + AF_NOPROG));
            continue;
          }
...
int
do_set_main_reactor(double level, dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no M/A reactor.", Name(sdb[n].object))));
...
int
do_set_aux_reactor(double level, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].aux.exist) {
    notify(enactor,
           ansi_red(tprintf("%s has no fusion reactor.", Name(sdb[n].object))));
    ...
    if (sdb[n].aux.in > sdb[n].aux.damage) {
      console_message(
        ...
        ansi_cmd(enactor, tprintf("Fusion reactor set at %.3f%% %s%sOVERLOAD%s",
                                  sdb[n].aux.in * 100.0, ANSI_BLINK, ANSI_RED,
                                  ANSI_NORMAL)));
    } else {
      console_message(n, "engineering",
                      ansi_cmd(enactor, tprintf("Fusion reactor set at %.3f%%",
                                                sdb[n].aux.in * 100.0)));
    }
    return 1;
  }
...
int
do_set_battery(double level, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].batt.exist) {
    notify(enactor,
           ansi_red(tprintf("%s has no batteries.", Name(sdb[n].object))));
    ...
    if (level < 0.0) {
      ...
      sdb[n].batt.in = level;
    console_message(n, "engineering",
                    ansi_cmd(enactor, tprintf("Batteries set at %.3f%%",
                                              sdb[n].batt.in * 100)));
    return 1;
  }
  return 0;
...
int
do_set_lrs(int active, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
...
int
do_set_srs(int active, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].sensor.srs_exist) {
    notify(enactor, ansi_red(tprintf("%s has no short-range sensors.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].sensor.srs_damage <= 0.0) {
    notify(enactor, ansi_red("Short-range sensors are inoperative."));
  } else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
    notify(enactor,
           ansi_red(tprintf("%s cannot use short-range sensors while cloaked.",
                            Name(sdb[n].object))));
  } else {
    if (active) {
...
int
do_set_ew(int active, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].sensor.ew_exist) {
    notify(enactor, ansi_red(tprintf("%s has no electronic warfare systems.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].sensor.ew_damage <= 0.0) {
    notify(enactor, ansi_red("Electronic warfare systems are inoperative."));
  } else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
    notify(enactor, ansi_red(tprintf(
                      "%s cannot use electronic warfare sensors while cloaked.",
                      Name(sdb[n].object))));
  } else {
    if (active) {
      if (sdb[n].sensor.ew_active) {
        notify(enactor,
               ansi_red("Electronic warfare systems are already online."));
      } else {
      ...
      if (!sdb[n].sensor.ew_active) {
        notify(enactor,
               ansi_red("Electronic warfare systems are already offline."));
      } else {
        sdb[n].sensor.ew_active = 0;
        sdb[n].sensor.version = 1;
        console_message(
          n, "helm science tactical",
...
int
do_set_cloak(int active, dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s has no cloaking device.",
                                     Name(sdb[n].object))));
    return 0;
      ...
      if (sdb[n].shield.exist) {
        ...
        for (i = 0; i < MAX_SHIELD_NAME; ++i) {
          if (sdb[n].shield.active[i]) {
            ...
            strncat(buffer,
                    ansi_cmd(enactor, tprintf("%s offline", unparse_shield(i))),
                    sizeof(buffer) - 1);
            ++flag;
          }
        }
...
int
do_set_course(double yaw, double pitch, double roll, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (sdb[n].status.docked) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s is still connected.", Name(sdb[n].object))));
  } else if (!sdb[n].engine.warp_exist && !sdb[n].engine.impulse_exist) {
    notify(enactor,
           ansi_red(tprintf("%s cannot be maneuvered.", Name(sdb[n].object))));
    ...
    if (sdb[n].status.autopilot) {
      ...
      console_message(n, "helm", ansi_cmd(enactor, "Autopilot disengaged"));
    }
    ...
    if (sdb[n].course.roll_in != 0.0) {
      console_message(
        n, "helm",
        ansi_cmd(enactor,
                 tprintf("Course set to %.3f %.3f %.3f", sdb[n].course.yaw_in,
                         sdb[n].course.pitch_in, sdb[n].course.roll_in)));
    } else {
      console_message(n, "helm",
                      ansi_cmd(enactor, tprintf("Course set to %.3f %.3f",
                                                sdb[n].course.yaw_in,
                                                sdb[n].course.pitch_in)));
    }
    return 1;
  }
  return 0;
...
int
do_set_yaw(double yaw, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (sdb[n].status.docked) {
    ...
    if (sdb[n].course.yaw_in < 0.0)
      sdb[n].course.yaw_in += 360.0;
    console_message(n, "helm",
                    ansi_cmd(enactor, tprintf("Yaw adjusted %+.3f to %.3f", yaw,
                                              sdb[n].course.yaw_in)));
    return 1;
  }
  return 0;
...
int
do_set_pitch(double pitch, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (sdb[n].status.docked) {
...
int
do_set_roll(double roll, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (sdb[n].status.docked) {
...
int
do_set_intercept(int contact, dbref enactor)
{
  int x;
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      if (sdb[n].course.roll_in != 0.0) {
        console_message(
          n, "helm",
          ansi_cmd(enactor,
                   tprintf("Intercept course to %s set %.3f %.3f %.3f",
                           unparse_identity(n, x), sdb[n].course.yaw_in,
                           sdb[n].course.pitch_in, sdb[n].course.roll_in)));
      } else {
        console_message(
          n, "helm",
          ansi_cmd(enactor,
                   tprintf("Intercept course to %s set %.3f %.3f",
                           unparse_identity(n, x), sdb[n].course.yaw_in,
                           sdb[n].course.pitch_in)));
      }
      return 1;
...
int
do_set_evade(int contact, dbref enactor)
{
  int x;
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      if (sdb[n].course.roll_in != 0.0) {
        console_message(
          n, "helm",
          ansi_cmd(enactor,
                   tprintf("Evasive course from %s set %.3f %.3f %.3f",
                           unparse_identity(n, x), sdb[n].course.yaw_in,
                           sdb[n].course.pitch_in, sdb[n].course.roll_in)));
      } else {
        console_message(
          n, "helm",
          ansi_cmd(enactor,
                   tprintf("Evasive course from %s set %.3f %.3f",
                           unparse_identity(n, x), sdb[n].course.yaw_in,
                           sdb[n].course.pitch_in)));
      }
      return 1;
...
int
do_set_parallel(int contact, dbref enactor)
{
  int x;
...
int
do_set_coords_manual(double x, double y, double z, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
    ...
    console_message(
      n, "helm",
      ansi_cmd(enactor,
               tprintf("Relative coordinates set to %.3f %.3f %.3f", x, y, z)));
    return 1;
  }
  return 0;
}
...
int
do_set_coords_reset(dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
    ...
    console_message(
      ...
      ansi_cmd(enactor, tprintf("Relative coordinates reset to %.3f %.3f %.3f",
                                su2pc(sdb[n].coords.x), su2pc(sdb[n].coords.y),
                                su2pc(sdb[n].coords.z))));
    return 1;
  }
  return 0;
}
...
int
do_set_coords_layin(double x, double y, double z, dbref enactor)
{
  //	double dx, dy, dz;
  ...
  if (error_on_console(enactor)) {
    ...
    console_message(
      n, "helm",
      ansi_cmd(enactor,
               tprintf("Coordinates %.3f %.3f %.3f laid in", x, y, z)));
    return 1;
  }
  return 0;
}
...
int
do_set_coords_engage(dbref enactor)
{
  //	double dx, dy, dz;
  ...
  if (error_on_console(enactor)) {
    ...
    sdb[n].course.yaw_in = xy2bearing(sdb[n].coords.xd - sdb[n].coords.x,
                                      sdb[n].coords.yd - sdb[n].coords.y);
    sdb[n].course.pitch_in = xyz2elevation(sdb[n].coords.xd - sdb[n].coords.x,
                                           sdb[n].coords.yd - sdb[n].coords.y,
                                           sdb[n].coords.zd - sdb[n].coords.z);
    console_message(
      n, "helm",
      ansi_cmd(enactor, tprintf("Course %.3f %.3f engaged",
...
int
do_set_eta(double speed, dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no engines.", Name(sdb[n].object))));
    return 0;
  }
  ...
  if (speed == 0.0) {
    ...
    if (speed == 0.0) {
      notify(enactor,
             ansi_red(tprintf("%s has no motion.", Name(sdb[n].object))));
      return 0;
    }
  }
  ...
  if (speed < 1.0) {
    ...
    speed =
      LIGHTSPEED * pow(speed, 3.333333) * 0.5 *
...
int
do_set_speed(double speed, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (sdb[n].status.docked) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no warp engines.", Name(sdb[n].object))));
  } else if (!sdb[n].engine.impulse_exist && (fabs(speed) < 1.0)) {
    notify(enactor, ansi_red(tprintf("%s has no impulse engines.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].engine.warp_damage <= 0.0 && (fabs(speed) >= 1.0)) {
    notify(enactor, ansi_red("Warp engines are inoperative."));
  } else if (sdb[n].engine.impulse_damage <= 0.0 && (speed != 0.0) &&
             (fabs(speed) < 1.0)) {
    ...
    if (fabs(sdb[n].move.in) >= 1.0) {
      if (fabs(sdb[n].move.in) > sdb[n].engine.warp_cruise) {
        console_message(
          n, "helm engineering",
          ansi_cmd(enactor,
                   tprintf("Speed set to warp %.6f %s%sOVERLOAD%s",
                           sdb[n].move.in, ANSI_BLINK, ANSI_RED, ANSI_NORMAL)));
      } else
        console_message(
          n, "helm engineering",
          ansi_cmd(enactor, tprintf("Speed set to warp %.6f", sdb[n].move.in)));
    } else {
      if (fabs(sdb[n].move.in) > sdb[n].engine.impulse_cruise) {
        console_message(
          n, "helm engineering",
          ansi_cmd(enactor,
                   tprintf("Speed set to %.3f%% impulse %s%sOVERLOAD%s",
                           sdb[n].move.in * 100.0, ANSI_BLINK, ANSI_RED,
                           ANSI_NORMAL)));
      } else
...
int
do_set_shield(int shield, int active, dbref enactor)
{
  static char buffer[800];
  int a, b, flag;
  register int i;
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no shields.", Name(sdb[n].object))));
    return 0;
  } else if (sdb[n].status.docked && active) {
    notify(enactor, ansi_red(tprintf("%s cannot raise shields while docked.",
                                     Name(sdb[n].object))));
    return 0;
  } else if (sdb[n].status.landed && active) {
    notify(enactor, ansi_red(tprintf("%s cannot raise shields while landed.",
                                     Name(sdb[n].object))));
    return 0;
  } else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
    notify(enactor, ansi_red(tprintf("%s cannot raise shields while cloaked.",
                                     Name(sdb[n].object))));
    return 0;
  }
  ...
  for (i = a; i <= b; ++i)
    if (sdb[n].shield.damage[i] <= 0.0) {
      notify(enactor,
             ansi_red(tprintf("%s is inoperative.", unparse_shield(i))));
    } else if (sdb[n].shield.active[i] && active) {
      notify(enactor,
             ansi_red(tprintf("%s is already online.", unparse_shield(i))));
    } else if (!sdb[n].shield.active[i] && !active) {
      notify(enactor,
             ansi_red(tprintf("%s is already offline.", unparse_shield(i))));
    } else {
      if (flag)
        strncat(buffer, "\n", sizeof(buffer) - 1);
      if (active) {
        ...
        strncat(buffer,
                ansi_cmd(enactor, tprintf("%s online", unparse_shield(i))),
                sizeof(buffer) - 1);
      } else {
        sdb[n].shield.active[i] = 0;
        strncat(buffer,
...
int
do_set_weapon(int first, int last, int active, int weapon, dbref enactor)
{
  ...
  if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
    ...
    for (i = a; i <= b; ++i)
      if (sdb[n].blist.damage[i] <= 0.0) {
        notify(enactor,
               ansi_red(tprintf("%s %d is inoperative.",
                                unparse_beam(sdb[n].blist.name[i]), i + 1)));
      } else if (active && sdb[n].blist.active[i]) {
        notify(enactor,
               ansi_red(tprintf("%s %d is already online.",
                                unparse_beam(sdb[n].blist.name[i]), i + 1)));
      } else if (!active && !sdb[n].blist.active[i]) {
        notify(enactor,
               ansi_red(tprintf("%s %d is already offline.",
                                unparse_beam(sdb[n].blist.name[i]), i + 1)));
      } else if (active) {
        sdb[n].blist.lock[i] = 0;
...
do_fed_shield_bug_check(int x)
{
  ...
  for (i = 0; i < MAX_SHIELD_NAME; ++i)
    if (sdb[x].shield.damage[i] > 0.0) {
      if (!sdb[x].shield.active[i]) {
        ...
        strncat(buffer, ansi_warn(tprintf("%s online", unparse_shield(i))),
                sizeof(buffer) - 1);
      }
      ++flag;
    }
  ...
  if (flag) {
    ...
    write_spacelog(sdb[x].object, sdb[x].object,
                   "LOG: Shields raised automagically");
...
int
do_lock_weapon(int first, int last, int contact, int weapon, dbref enactor)
{
  static char buffer[BUFFER_LEN];
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s cannot lock weapons while cloaked.",
                                     Name(sdb[n].object))));
    return 0;
  }
  ...
  if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
    ...
    for (i = a; i <= b; ++i)
      if (sdb[n].blist.damage[i] <= 0.0) {
        ...
        notify(enactor,
               ansi_red(tprintf("%s %d is not online.",
                                unparse_beam(sdb[n].blist.name[i]), i + 1)));
      } else if (sdb[n].blist.lock[i]) {
        notify(enactor,
               ansi_red(tprintf("%s %d is already locked.",
                                unparse_beam(sdb[n].blist.name[i]), i + 1)));
      } else {
  ...
  if (flag_b || flag_m) {
    console_message(n, "tactical",
                    ansi_cmd(enactor, tprintf("%s locked on %s", buffer,
                                              unparse_identity(n, x))));
    if (!flag_lock) {
      y = sdb2contact(x, n);
      if (y != SENSOR_FAIL)
        console_message(x, "helm science tactical",
                        ansi_warn(tprintf("%cWeapon lock from %s detected",
                                          BEEP_CHAR, unparse_identity(x, n))));
    }
  }
...
int
do_unlock_weapon(int first, int last, int weapon, dbref enactor)
{
  static char buffer[BUFFER_LEN];
  int a, b, flag_b = 0, flag_m = 0;
  // int flag_lock = 0, x;
  ...
  if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
    ...
    for (i = a; i <= b; ++i)
      if (sdb[n].blist.damage[i] <= 0.0) {
        ...
        notify(enactor,
               ansi_red(tprintf("%s %d is already unlocked.",
                                unparse_beam(sdb[n].blist.name[i]), i + 1)));
      } else {
        sdb[n].blist.lock[i] = 0;
        if (!flag_b)
...
int
do_set_trans(int active, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].trans.exist) {
    notify(enactor,
           ansi_red(tprintf("%s has no transporters.", Name(sdb[n].object))));
  } else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
    notify(enactor,
           ansi_red(tprintf("%s cannot use transporters while cloaked.",
                            Name(sdb[n].object))));
  } else {
    if (active) {
...
int
do_set_trans_locked(int contact, int target, dbref enactor)
{
  int x, y;
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      if (sdb[n].trans.damage <= 0.0) {
        ...
        if (target) {
          ...
          console_message(
            n, "operation transporter",
            ansi_cmd(enactor, tprintf("Transporter dest lock on %s",
                                      unparse_identity(n, x))));
        } else {
          sdb[n].trans.s_lock = x;
          console_message(
            n, "operation transporter",
            ansi_cmd(enactor, tprintf("Transporter source lock on %s",
                                      unparse_identity(n, x))));
        }
      }
      if (x != n) {
        ...
        if (y != SENSOR_FAIL)
          console_message(
            x, "operation tactical transporter",
            ansi_warn(tprintf("%cTransporter lock from %s detected", BEEP_CHAR,
                              unparse_identity(x, n))));
      }
      return 1;
    }
  }
...
int
do_set_trans_unlocked(dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].trans.exist) {
...
int
do_set_tract(int active, dbref enactor)
{
  int x;
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no tractor beams.", Name(sdb[n].object))));
  } else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
    notify(enactor,
           ansi_red(tprintf("%s cannot use tractor beams while cloaked.",
                            Name(sdb[n].object))));
  } else {
    if (active) {
...
int
do_set_tract_locked(int contact, dbref enactor)
{
  int x, c;
  double pn, px, pc;
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no tractor beam.", Name(sdb[n].object))));
  } else if (sdb[n].status.docked) {
    notify(enactor, ansi_red(tprintf("%s is in dock.", Name(sdb[n].object))));
  } else if (sdb[n].status.landed) {
    notify(enactor,
    ...
    notify(enactor,
           ansi_red(tprintf("%s is already tractoring %s.", Name(sdb[n].object),
                            Name(sdb[sdb[n].status.tractoring].object))));
  } else if (sdb[n].status.tractored) {
    notify(enactor,
           ansi_red(tprintf("%s is being tractored by %s.", Name(sdb[n].object),
                            Name(sdb[sdb[n].status.tractored].object))));
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      if ((pn = sdb[n].tract.damage * sdb[n].power.total *
                sdb[n].alloc.tractors / (sdb2range(n, x) + 1.0)) < 1.0) {
        notify(enactor, ansi_red("Tractor beam is not sufficiently powered."));
      } else {
        if (sdb[x].status.tractored) {
          c = sdb[x].status.tractored;
          pc = sdb[c].tract.damage * sdb[c].power.total *
               sdb[c].alloc.tractors / (sdb2range(c, x) + 1.0);
          px = sdb[x].tract.damage * sdb[x].power.total *
               sdb[x].alloc.tractors / (sdb2range(n, x) + 1.0);
          if (pn > pc && pn > px) {
...
int
do_set_tract_unlocked(dbref enactor)
{
  int x;
...
int
do_set_ftr_alloc(double helm, double tactical, double operations, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
...
int
do_set_eng_alloc(double helm, double tactical, double operations, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
...
int
do_set_helm_alloc(double movement, double shields, double cloak, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
...
int
do_set_shield_alloc(double forward, double starboard, double aft, double port,
                    double dorsal, double ventral, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
...
int
do_set_tactical_alloc(double beams, double missiles, double sensors,
                      dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
...
int
do_set_sensor_alloc(double ecm, double eccm, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else {
...
int
do_set_operations_alloc(double transporters, double tractors,
...
do_set_active(dbref enactor)
{
  ...
  if (sdb[n].structure.type == 0) {
    ...
    notify(enactor, ansi_red(tprintf("%s systems are already active.",
                                     Name(sdb[n].object))));
  } else {
    do_all_console_notify(
      n, ansi_cmd(enactor, "All systems initializing and starting up"));
    do_ship_notify(n,
                   tprintf("%s activates all systems.", Name(sdb[n].object)));
...
do_set_inactive(dbref enactor)
{
  ...
  if (sdb[n].structure.type == 0) {
    ...
    notify(enactor, ansi_red(tprintf("%s systems are already inactive.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].power.total != 0) {
    notify(enactor, ansi_red(tprintf("%s power systems are still online.",
                                     Name(sdb[n].object))));
  } else {
    do_all_console_notify(n, ansi_cmd(enactor, "All systems shutting down"));
    do_ship_notify(n,
                   tprintf("%s deactivates all systems.", Name(sdb[n].object)));
    do_space_notify_one(n, console_helm, console_tactical, console_science,
...
int
do_set_docking_doors(int active, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].structure.has_docking_bay) {
    notify(enactor,
           ansi_red(tprintf("%s has no docking bay.", Name(sdb[n].object))));
  } else {
    if (active) {
...
int
do_set_landing_doors(int active, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].structure.has_landing_pad) {
    notify(enactor,
           ansi_red(tprintf("%s has no landing bay.", Name(sdb[n].object))));
  } else {
    if (active) {
...
int
do_set_dock(int contact, dbref enactor)
{
  register int i;
  int x, l = sdb[n].location;
  // int s;
  /* ASPACE TRAC DOCK CHANGES n is current ship, x is the dock
   * sdb[n].status.tractoring is the tractoree */
  ...
  if (trac) {
    ...
    // tloc = sdb[sdb[n].status.tractoring].location; /* Removed due to not
    // being used. */
  }
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s is incapable of docking.",
                                     Name(sdb[n].object))));
    /* check if the tractoree can dock   ok'd 8/23/00 */
  } else if (trac && !sdb[trac].structure.can_dock) {
    notify(enactor,
           ansi_red(tprintf(
             "The ship you are tractoring (%s) is incapable of docking.",
             Name(sdb[trac].object))));
  } else if (sdb[n].status.connected) {
    notify(enactor,
           ansi_red(tprintf("%s is still connected.", Name(sdb[n].object))));
  } else if (sdb[n].status.tractored) {
    notify(enactor, ansi_red(tprintf("%s cannot dock while being tractored.",
                                     Name(sdb[n].object))));
    /* We don't need this really, but i'll leave it just in case you want to
     * revert */
    /*	} else if (sdb[n].status.tractoring) {
                    notify(enactor, ansi_red(tprintf("%s cannot dock while using
       its tractor beam.", Name(sdb[n].object)))); */
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      notify(enactor, ansi_red(tprintf("%s does not have a dock.",
                                       Name(sdb[x].object))));
    } else if (!sdb[x].status.open_docking) {
      notify(enactor, ansi_red(tprintf("%s's docking bay doors are closed.",
                                       Name(sdb[x].object))));
      /* check size on dock. include the tractoree! */
    } else if ((tsize + sdb[n].structure.displacement +
                sdb[x].structure.cargo_mass) > sdb[x].structure.cargo_hold) {
      notify(enactor, ansi_red(tprintf("%s does not have enough room.",
                                       Name(sdb[x].object))));
    } else if (sdb[n].shield.active[0] || sdb[n].shield.active[1] ||
               sdb[n].shield.active[2] || sdb[n].shield.active[3] ||
               sdb[n].shield.active[4] || sdb[n].shield.active[5]) {
      notify(enactor, ansi_red(tprintf("%s must lower all shields to dock.",
                                       Name(sdb[n].object))));
    } else if (sdb2dissipation(x, sdb2shield(x, n)) != 0.0) {
      notify(enactor, ansi_red(tprintf("%s's facing shield must be lowered.",
                                       Name(sdb[x].object))));
    } else if (sdb[x].status.tractored) {
      notify(enactor,
             ansi_red(tprintf("%s is being tractored.", Name(sdb[x].object))));
    } else {
      if (sdb[n].status.docked) {
        console_message(
          n, "helm science tactical",
          ansi_cmd(enactor, tprintf("%s undocking from %s", Name(sdb[n].object),
                                    Name(sdb[l].object))));
        console_message(
          l, "helm science tactical",
          ansi_alert(tprintf("%s undocking from %s", Name(sdb[n].object),
                             Name(sdb[l].object))));
        do_ship_notify(
          n, tprintf("The %s slides out of dock.", Name(sdb[n].object)));
        do_space_notify_two(n, l, console_helm, console_tactical,
                            console_science, "undocking from");
      } else if (sdb[n].status.landed) {
        console_message(
          n, "helm science tactical",
          ansi_cmd(enactor, tprintf("%s launching from %s", Name(sdb[n].object),
                                    Name(sdb[l].object))));
        console_message(
          l, "helm science tactical",
          ansi_alert(tprintf("%s launching from %s", Name(sdb[n].object),
                             Name(sdb[l].object))));
        do_ship_notify(n, tprintf("The %s lifts off from its landing pad.",
                                  Name(sdb[n].object)));
        do_space_notify_two(n, l, console_helm, console_tactical,
                            console_science, "launching from");
      }
      ...
      if (trac)
        moveit(sdb[sdb[n].status.tractoring].object, sdb[x].object, 1, enactor,
               "aspace");

      console_message(
        n, "helm science tactical",
        ansi_cmd(enactor, tprintf("%s docking with %s", Name(sdb[n].object),
                                  Name(sdb[x].object))));
      console_message(
        x, "helm science tactical",
        ansi_alert(tprintf("%s docking with %s", Name(sdb[n].object),
                           Name(sdb[x].object))));
      do_ship_notify(n,
                     tprintf("The %s slides into dock.", Name(sdb[n].object)));
      ...
      if (trac) {
        ...
        console_message(
          n, "helm science tactical",
          ansi_cmd(enactor, tprintf("%s tractors %s into dock with %s",
                                    Name(sdb[n].object), Name(sdb[trac].object),
                                    Name(sdb[x].object))));
        console_message(
          x, "helm science tactical",
          ansi_alert(tprintf("%s tractors %s into dock with %s",
                             Name(sdb[n].object), Name(sdb[trac].object),
                             Name(sdb[x].object))));
        do_ship_notify(trac,
                       tprintf("The %s pulls %s into dock.",
                               Name(sdb[n].object), Name(sdb[trac].object)));
        do_space_notify_two(trac, x, console_helm, console_tactical,
                            console_science, "docking with");
      }
      ...
      sdb[n].course.pitch_in = sdb[n].course.pitch_out =
      ...
      if (trac) {
        ...
        sdb[trac].course.yaw_in = sdb[trac].course.yaw_out =
          sdb[x].course.yaw_out;
        sdb[trac].course.pitch_in = sdb[trac].course.pitch_out =
          sdb[x].course.pitch_out;
        sdb[trac].course.roll_in = sdb[trac].course.roll_out =
...
do_set_undock(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s is not in dock.", Name(sdb[n].object))));
  } else if (sdb[n].status.connected) {
    notify(enactor,
           ansi_red(tprintf("%s is still connected.", Name(sdb[n].object))));
      ...
      if (l > 0 && l <= max_space_objects) {
        ...
        console_message(
          ...
          ansi_alert(tprintf("%s docking with %s", Name(sdb[n].object),
                             Name(sdb[l].object))));
        do_ship_notify(
          n, tprintf("The %s slides into dock.", Name(sdb[n].object)));
        do_space_notify_two(n, l, console_helm, console_tactical,
                            console_science, "docking with");
        sdb[l].structure.cargo_mass += sdb[n].structure.displacement;
        sdb[l].engine.version = 1;
...
int
do_set_land(int contact, dbref enactor)
{
  register int i;
  int x, l = sdb[n].location;
  // int s;
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s is incapable of landing.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].status.connected) {
    notify(enactor,
           ansi_red(tprintf("%s is still connected.", Name(sdb[n].object))));
  } else if (sdb[n].status.tractored) {
    notify(enactor, ansi_red(tprintf("%s cannot land while being tractored.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].status.tractoring) {
    notify(enactor,
           ansi_red(tprintf("%s cannot land while using its tractor beam.",
                            Name(sdb[n].object))));
  } else if (fabs(sdb[n].move.out) >= 1.0) {
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      notify(enactor, ansi_red(tprintf("%s does not have a landing pad.",
                                       Name(sdb[x].object))));
    } else if (!sdb[x].status.open_landing) {
      notify(enactor, ansi_red(tprintf("%s's landing bay doors are closed.",
                                       Name(sdb[x].object))));
    } else if ((sdb[n].structure.displacement + sdb[x].structure.cargo_mass) >
      ...
      notify(enactor, ansi_red(tprintf("%s does not have enough room.",
               ...
               sdb[n].shield.active[2] || sdb[n].shield.active[3] ||
               sdb[n].shield.active[4] || sdb[n].shield.active[5]) {
      notify(enactor, ansi_red(tprintf("%s must lower all shields to land.",
                                       Name(sdb[n].object))));
    } else if (sdb2dissipation(x, sdb2shield(x, n)) != 0.0) {
      notify(enactor, ansi_red(tprintf("%s's facing shield must be lowered.",
                                       Name(sdb[x].object))));
      ...
      console_message(
        n, "helm science tactical",
        ansi_cmd(enactor, tprintf("%s landing on %s", Name(sdb[n].object),
                                  Name(sdb[x].object))));
      console_message(
        x, "helm science tactical",
        ansi_alert(tprintf("%s landing on %s", Name(sdb[n].object),
                           Name(sdb[x].object))));
      do_ship_notify(n, tprintf("The %s settles onto its landing pad.",
                                Name(sdb[n].object)));
      do_space_notify_two(n, x, console_helm, console_tactical, console_science,
                          "landing on");
...
do_set_launch(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s is not on a landing pad.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].status.connected) {
    notify(enactor,
           ansi_red(tprintf("%s is still connected.", Name(sdb[n].object))));
...
up_wormhole(int n1, int n2)
{
  ...
  do_ship_notify(
    n, tprintf("The %s shudders and rocks about violently for a few moments.",
               Name(sdb[n].object)));
  do_space_notify_two(n, x, console_helm, console_tactical, console_science,
                      "enters");
  if (sdb[n].cloak.active) {
  ...
  sdb[n].status.autopilot = 0;
  do_space_notify_one(sdb[x].status.link, console_helm, console_tactical,
                      console_science, "expells an unknown contact");
  up_cochranes();
  up_empire(n);
  up_quadrant();
  up_resolution();
...
int
do_set_enter_wormhole(int contact, dbref enactor)
{
  int x, t;
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor, ansi_red(tprintf("%s is much too large to enter.",
                                     Name(sdb[n].object))));
  } else if (sdb[n].status.docked) {
    notify(enactor, ansi_red(tprintf("%s is in dock.", Name(sdb[n].object))));
  } else if (sdb[n].status.landed) {
    ...
    if (error_on_contact(n, x, enactor)) {
      ...
      notify(enactor,
             ansi_red(tprintf("%s is moving too fast.", Name(sdb[x].object))));
    } else if (!SpaceObj(sdb[sdb[x].status.link].object) ||
               !GoodObject(sdb[sdb[x].status.link].object)) {
      ...
      if (t) {
        if (sdb[t].structure.type != 1) {
          ...
          notify(enactor, ansi_red(tprintf("%s is too far away to enter.",
                                           Name(sdb[t].object))));
          return 0;
        } else
          up_wormhole(t, x);
      ...
      do_all_console_notify(
        n, ansi_cmd(enactor, tprintf("%s entering %s", Name(sdb[n].object),
                                     Name(sdb[x].object))));
      up_wormhole(n, x);
      return 1;
    }
...
int
do_set_fix_damage(char *sys1, char *sys2, int type, char *name, dbref enactor)
{
  ...
  if (!GoodSDB(n)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s systems are inactive.", Name(sdb[n].object))));
    return 0;
  } else if (sdb[n].structure.max_repair <= 0) {
    notify(enactor, ansi_red(tprintf("%s has no repair capacity.",
                                     Name(sdb[n].object))));
    return 0;
  }
  ...
  if (type) {
    ...
    if (x == SENSOR_FAIL) {
      ...
      notify(enactor,
             ansi_red(tprintf("%s is not connected.", Name(sdb[x].object))));
      return 0;
  ...
  case 'A':
    ...
    default:
      notify(enactor, ansi_red("Invalid system specification."));
      return 0;
      break;
    }
    break;
  ...
  if (num) {
    ...
    case 2:
      ...
      break;
    case 3:
      mult = sdb[x].blist.cost[unit - 1];
      break;
    case 8:
      mult = sdb[x].main.gw;
      break;
    case 9:
      mult = sdb[x].mlist.warhead[unit - 1];
      break;
    default:
      mult = (1.0 + (sdb[x].structure.max_structure / 10.0));
      break;
    }
    ...
    if (fix_cost > sdb[n].structure.repair) {
      *dmg += sdb[n].structure.repair / repair_mult[num] / mult / 100.0;
      ...
      console_message(
        n, "damage",
        ansi_cmd(enactor, tprintf("%s repaired to %s", system_name[num],
                                  unparse_percent(*dmg))));
      if (type)
        console_message(
          x, "damage",
          ansi_alert(tprintf("%s repaired to %s by %s", system_name[num],
                             unparse_percent(*dmg), Name(sdb[n].object))));
    } else {
      *dmg = 1.0;
      ...
      if (type)
        console_message(
          x, "damage",
          ansi_alert(tprintf("%s repairs completed by %s", system_name[num],
                             Name(sdb[n].object))));
    }
    ...
    if (sdb[x].structure.superstructure < 0.0)
      flag = 1;
    fix_cost =
      (sdb[x].structure.max_structure - sdb[x].structure.superstructure) *
      repair_mult[0];
    if (fix_cost > sdb[n].structure.repair) {
      sdb[x].structure.superstructure +=
        sdb[n].structure.repair / repair_mult[0];
      sdb[n].structure.repair = 0.0;
      console_message(
        n, "damage",
        ansi_cmd(enactor,
                 tprintf("%s repaired to %s", system_name[0],
                         unparse_percent(sdb[x].structure.superstructure /
...
int
do_set_shield_freq(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].shield.exist) {
    ...
    notify(enactor,
           ansi_red("Valid shield frequencies are from 1.00 to 999.999 GHz."));
  } else {
    ...
    write_spacelog(
      enactor, sdb[n].object,
      tprintf("LOG: Shield frequencies set to %.6f GHz", sdb[n].shield.freq));
    return 1;
  }
  return 0;
}
...
int
do_set_cloak_freq(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].cloak.exist) {
    ...
    notify(
      enactor,
      ansi_red(
        "Valid cloaking device frequencies are from 1.00 to 999.999 GHz."));
  } else {
    ...
    console_message(
      ...
      ansi_cmd(enactor, tprintf("Cloaking device frequency set to %.3f GHz",
                                sdb[n].cloak.freq)));
    write_spacelog(enactor, sdb[n].object,
                   tprintf("LOG: Cloaking device frequency set to %.6f GHz",
                           sdb[n].cloak.freq));
    return 1;
  }
  return 0;
...
int
do_set_beam_freq(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].beam.exist) {
    notify(enactor,
           ansi_red(tprintf("%s has no beam weapons.", Name(sdb[n].object))));
  } else if (frequency < 1.0 || frequency >= 1000.0) {
    notify(
      enactor,
      ansi_red("Valid beam weapon frequencies are from 1.00 to 999.999 GHz."));
  } else {
    sdb[n].beam.freq = frequency;
    console_message(
      n, "engineering tactical",
      ansi_cmd(enactor, tprintf("Beam weapon frequencies set to %.3f GHz",
                                sdb[n].beam.freq)));
    write_spacelog(enactor, sdb[n].object,
                   tprintf("LOG: Beam weapon frequencies set to %.6f GHz",
                           sdb[n].beam.freq));
    return 1;
  }
  return 0;
...
int
do_set_missile_freq(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].missile.exist) {
    notify(enactor, ansi_red(tprintf("%s has no missile weapons.",
                                     Name(sdb[n].object))));
  } else if (frequency < 1.0 || frequency >= 1000.0) {
    notify(enactor,
           ansi_red(
             "Valid missile weapon frequencies are from 1.00 to 999.999 GHz."));
  } else {
    sdb[n].missile.freq = frequency;
    console_message(
      n, "engineering tactical",
      ansi_cmd(enactor, tprintf("Missile weapon frequencies set to %.3f GHz",
                                sdb[n].missile.freq)));
    write_spacelog(enactor, sdb[n].object,
                   tprintf("LOG: Missile weapon frequencies set to %.6f GHz",
                           sdb[n].missile.freq));
    return 1;
  }
  return 0;
...
int
do_set_trans_freq(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].trans.exist) {
    ...
    notify(
      enactor,
      ansi_red("Valid transporter frequencies are from 1.00 to 999.999 GHz."));
  } else {
    sdb[n].trans.freq = frequency;
    console_message(
      n, "engineering operation transporter",
      ansi_cmd(enactor, tprintf("Transporter frequency set to %.3f GHz",
                                sdb[n].trans.freq)));
    write_spacelog(
      enactor, sdb[n].object,
      tprintf("LOG: Transporter frequency set to %.6f GHz", sdb[n].trans.freq));
    return 1;
  }
  return 0;
}
...
int
do_set_tract_freq(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (!sdb[n].tract.exist) {
    ...
    notify(
      enactor,
      ansi_red("Valid tractor beam frequency is from 1.00 to 999.999 GHz."));
  } else {
    sdb[n].tract.freq = frequency;
    console_message(
      n, "engineering operation",
      ansi_cmd(enactor, tprintf("Tractor beam frequency set to %.3f GHz",
                                sdb[n].tract.freq)));
    write_spacelog(enactor, sdb[n].object,
                   tprintf("LOG: Tractor beam frequency set to %.6f GHz",
                           sdb[n].tract.freq));
    return 1;
  }
  return 0;
...
int
do_set_refuel(char *name, char *fuel, double tons, dbref enactor)
{
  int x, f;
  ...
  if (x == SENSOR_FAIL) {
    ...
    if (amount > available) {
      notify(enactor,
             ansi_red(tprintf("%s has only %f tons of antimatter available",
      ...
      notify(enactor,
             ansi_red(tprintf("%s has only %f tons of antimatter capacity",
    ...
    if (amount > available) {
      notify(enactor,
             ansi_red(tprintf("%s has only %f tons of deuterium available",
                              Name(sdb[n].object), available / 1000000.0)));
    } else if (capacity <= 0) {
      ...
      notify(enactor,
             ansi_red(tprintf("%s has only %f tons of deuterium capacity",
                              Name(sdb[x].object), capacity / 1000000.0)));
    } else {
    ...
    if (amount > available) {
      notify(enactor,
             ansi_red(tprintf("%s has only %f GW hours of reserves available",
      ...
      notify(enactor,
             ansi_red(tprintf("%s has only %f GW hours of reserves capacity",
...
int
do_set_defuel(char *fuel, double tons, dbref enactor)
{
  int x, f;
  double amount, capacity, available;
  ...
  if (GoodSDB(x)) {
    if (!sdb[n].status.connected) {
      notify(enactor,
             ansi_red(tprintf("%s is not connected.", Name(sdb[n].object))));
    } else if (f == 1) {
      amount = tons * 1000000.0;
      available = sdb[n].fuel.antimatter;
      capacity = sdb2max_antimatter(x) - sdb[x].fuel.antimatter;
...
int
do_set_autopilot(int flag, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (sdb[n].status.docked) {
    ...
    } else if (sdb[n].coords.xd == sdb[n].coords.x &&
               sdb[n].coords.yd == sdb[n].coords.y &&
               sdb[n].coords.zd == sdb[n].coords.z) {
      notify(enactor,
             ansi_red(tprintf("%s is already there.", Name(sdb[n].object))));
    } else {
      sdb[n].status.autopilot = 100;
      console_message(n, "helm", ansi_cmd(enactor, "Autopilot engaged"));
      return 1;
    }
  }
  return 0;
...
int
do_set_iff_frequency(double frequency, dbref enactor)
{
  if (error_on_console(enactor)) {
    return 0;
  } else if (frequency < 1.0 || frequency >= 1000.0) {
    notify(enactor,
           ansi_red("Valid IFF frequency is from 1.0 to 999.999 GHz."));
  } else {
    sdb[n].iff.frequency = frequency;
    console_message(n, "operation tactical",
                    ansi_cmd(enactor, tprintf("IFF Frequency set to %.3f GHz",
                                              sdb[n].iff.frequency)));
    write_spacelog(
      enactor, sdb[n].object,
      tprintf("LOG: IFF Frequency set to %.6f GHz", sdb[n].iff.frequency));
    return 1;
  }
  return 0;
}

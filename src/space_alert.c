...
void
alert_main_balance(int x)
{
  console_message(x, "engineering",
                  ansi_alert(tprintf("M/A reactor balanced at %.3f%%",
                                     sdb[x].main.out * 100.0)));
  return;
}
...
void
alert_aux_balance(int x)
{
  console_message(x, "engineering",
                  ansi_alert(tprintf("Fusion reactor balanced at %.3f%%",
                                     sdb[x].aux.out * 100.0)));
  return;
}
...
void
alert_warp_overload(int x)
{
  console_message(x, "engineering damage",
...
void
alert_impulse_overload(int x)
{
  console_message(
    x, "engineering damage",
...
void
alert_cloak_failure(int x)
{
  console_message(x, "engineering helm tactical",
...
void
alert_cloak_voided(int x)
{
  console_message(x, "helm tactical",
...
void
alert_missile_balance(int x)
{
  console_message(x, "tactical",
...
void
alert_speed_warp(int x)
{
  console_message(x, "engineering helm",
...
void
alert_tract_unlock(int n1, int n2, dbref enactor)
{
  console_message(n1, "helm operation science",
                  ansi_cmd(enactor, tprintf("Tractor beam unlocked from %s",
                                            unparse_identity(n1, n2))));
  console_message(n2, "helm operation science",
  ...
  do_ship_notify(
    n2, tprintf("%s is released from a tractor beam.", Name(sdb[n2].object)));
  return;
}
...
void
alert_tract_lock(int n1, int n2, dbref enactor)
{
  console_message(n1, "helm operation science",
  ...
  do_ship_notify(
    n2, tprintf("%s is seized by a tractor beam.", Name(sdb[n2].object)));
  return;
}
...
void
alert_tract_attempt(int n1, int n2, dbref enactor)
{
  console_message(
    n1, "helm operation science",
    ansi_cmd(enactor, tprintf("Tractor beam lock on %s attempted and failed",
                              unparse_identity(n1, n2))));
  console_message(
    n2, "helm operation science",
...
void
alert_tract_lost(int n1, int n2)
{
  console_message(n1, "helm operation science",
...
void
alert_ship_cloak_online(int x)
{
  do_ship_notify(
    x, tprintf("%s engages its cloaking device.", Name(sdb[x].object)));
...
void
alert_ship_cloak_offline(int x)
{
  do_ship_notify(
    x, tprintf("%s disengages its cloaking device.", Name(sdb[x].object)));
  do_space_notify_one(x, console_helm, console_tactical, console_science,
...
void
alert_ship_hit(int x)
{
  if (sdb[x].structure.type == 1 || sdb[x].structure.type == 2)
    do_ship_notify(x,
                   tprintf("%s shudders from an impact.", Name(sdb[x].object)));
  return;
}
...
void
alert_ship_hurt(int x)
{
  if (sdb[x].structure.type == 1 || sdb[x].structure.type == 2) {
    do_ship_notify(
      x, tprintf("%s rocks violently from an impact.", Name(sdb[x].object)));
  } else if (sdb[x].structure.type == 3 && get_random_long(0, 10) == 1)
    do_ship_notify(
      x, tprintf("%s trembles from a surface impact.", Name(sdb[x].object)));
  return;
}

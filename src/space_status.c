...
contact_flags(int x)
{
  ...
  if (sdb[x].status.docked)
    *fp++ = 'd';
  if ((sdb[x].power.total * sdb[x].alloc.sensors > 0.0) &&
  ...
  if (sdb[x].shield.exist)
    for (i = 0; i < MAX_SHIELD_NAME; ++i)
      if (sdb[x].shield.active[i] * sdb[x].alloc.shield[i] *
            sdb[x].power.total * sdb[x].shield.damage[i] >
          0.0) {
        *fp++ = 'S';
        break;
      }
...
contact_line(int contact)
{
  ...
  if (level < 25) {
    snprintf(
      ...
      "%s%3d%s %-4.4s %3.0f %3.0f %-3.0f %7.7s %-5.5s %3.0f %-3.0f %6.6s "
      "%-5.5s %s%s%s%s\n",
      ANSI_CYAN, sdb[n].slist.num[contact], ANSI_WHITE, unparse_type(x), level,
      sdb2bearing(n, x), sdb2elevation(n, x), unparse_range(sdb2range(n, x)),
      arc1, sdb[x].course.yaw_out, sdb[x].course.pitch_out,
      unparse_speed(sdb[x].move.out), arc2, ANSI_HILITE,
      (sdb[n].iff.frequency == sdb[x].iff.frequency ? ANSI_GREEN : ANSI_RED),
      "*", ANSI_NORMAL);
  } else if (level < 50.0) {
    snprintf(
      buffer, sizeof(buffer),
      "%s%3d%s %-4.4s %3.0f %3.0f %-3.0f %7.7s %-5.5s %3.0f %-3.0f %6.6s "
      "%-5.5s %s%s%s%s%16.16s %-6.6s\n",
      ANSI_CYAN, sdb[n].slist.num[contact], ANSI_WHITE, unparse_type(x), level,
      sdb2bearing(n, x), sdb2elevation(n, x), unparse_range(sdb2range(n, x)),
      arc1, sdb[x].course.yaw_out, sdb[x].course.pitch_out,
      unparse_speed(sdb[x].move.out), arc2, ANSI_HILITE,
      (sdb[n].iff.frequency == sdb[x].iff.frequency ? ANSI_GREEN : ANSI_RED),
      "*", ANSI_NORMAL, (sdb[x].cloak.active ? "(cloaked)" : unparse_class(x)),
      contact_flags(x));
  } else {
    snprintf(
      buffer, sizeof(buffer),
      "%s%3d%s %-4.4s %3.0f %3.0f %-3.0f %7.7s %-5.5s %3.0f %-3.0f %6.6s "
      "%-5.5s %s%s%s%s%s%-16.16s%s %-6.6s\n",
      ANSI_CYAN, sdb[n].slist.num[contact], ANSI_WHITE, unparse_type(x), level,
      sdb2bearing(n, x), sdb2elevation(n, x), unparse_range(sdb2range(n, x)),
      arc1, sdb[x].course.yaw_out, sdb[x].course.pitch_out,
      unparse_speed(sdb[x].move.out), arc2, ANSI_HILITE,
      ...
      (sdb[x].cloak.active ? "(cloaked)" : Name(sdb[x].object)), ANSI_NORMAL,
      contact_flags(x));
  }
  return (buffer);
}
...
int
do_sensor_contacts(char *a, dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             format_Course(n), format_Speed(n), format_Location(n),
             format_Velocity(n), format_l_end());
    ...
    snprintf(buffer, sizeof(buffer),
             "%s%s--[%sSensor "
             "Report%s]--------------------------------------------------------"
             "------%s\n%s### Type Res Bearing Range   Arcs  Heading Speed  "
             "Arcs  Name       Class flags%s\n%s--- ---- --- ------- ------- "
             "----- ------- ------ ----- ---------------- ------%s\n",
             ANSI_HILITE, ANSI_BLUE, ANSI_YELLOW, ANSI_BLUE, ANSI_NORMAL,
    ...
    strncat(buffer,
            tprintf("%s%s%s\n%s%s\n%s", format_l_line(), format_Course(n),
                    format_Speed(n), format_Location(n), format_Velocity(n),
                    format_l_end()),
            sizeof(buffer) - 1);
    notify(enactor, buffer);
    return 1;
    ...
    snprintf(buffer, sizeof(buffer),
             "%s### Type Res Bearing Range   Arcs  Heading Speed  Arcs  Name   "
             "    Class flags%s\n%s",
...
void
contact_line_bot(int contact, char *buff, char **bp)
{
  int x = sdb[n].slist.sdb[contact];
  ...
  safe_format(
    buff, bp,
    "%d|%d|#%d|%d|%.0f|%.3f|%.3f|%.1f|%.3f|%.3f|%f|%.1f %.1f "
    "%.1f|%s|%s|%s|%s|%s|%d",
    sdb[n].slist.num[contact], x, sdb[x].object, sdb[x].structure.type, level,
    sdb2bearing(n, x), sdb2elevation(n, x), sdb2range(n, x),
    sdb[x].course.yaw_out, sdb[x].course.pitch_out, sdb[x].move.out,
    sdb[x].coords.x - sdb[n].coords.xo, sdb[x].coords.y - sdb[n].coords.yo,
    sdb[x].coords.z - sdb[n].coords.zo, arc1, arc2,
    (level < 25 ? "" : contact_flags(x)),
    ((level < 50) && (sdb[x].structure.type < 3)
       ? ""
       : (sdb[x].cloak.active ? "(cloaked)" : unparse_class(x))),
    ((level < 50) && (sdb[x].structure.type < 3)
       ? ""
       : (sdb[x].cloak.active ? "(cloaked)" : Name(sdb[x].object))),
    sdb[x].move.empire);
}

void
do_sensor_contacts_bot(char *a, dbref enactor, char *buff, char **bp)
{
  register int contact = contact2slist(n, atoi(a));
  int ctype = 0, first = 0;
...
int
do_sensor_report(int contact, dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    notify(enactor,
           ansi_red(tprintf("%s has no sensors.", Name(sdb[n].object))));
  } else if (!sdb[n].sensor.contacts) {
    ...
    if (sdb[n].slist.lev[i] > 0.5 && !sdb[x].cloak.active) {
      ...
      strncat(buffer, "\n", sizeof(buffer) - 1);
      if (sdb[x].structure.type < 3 && (sdb[x].structure.has_docking_bay ||
...
do_eng_status(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    if (sdb[n].main.exist || sdb[n].aux.exist || sdb[n].batt.exist ||
        sdb[n].engine.warp_exist || sdb[n].engine.impulse_exist) {
      strncat(buffer, format_l_line(), sizeof(buffer) - 1);
...
do_helm_status(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             "Report%s]--------------------------------------------------------"
             "-%s%s\n",
             ANSI_BLUE, ANSI_HILITE, ANSI_YELLOW, ANSI_BLUE, ANSI_WHITE,
             ANSI_NORMAL);
...
int
do_missile_status(dbref enactor)
{
  static char buffer[BUFFER_LEN];
...
int
do_tactical_status(dbref enactor)
{
  static char buffer[BUFFER_LEN];
  ...
  if (error_on_console(enactor)) {
    ...
    if (sdb[n].sensor.lrs_exist || sdb[n].sensor.srs_exist ||
        sdb[n].sensor.ew_exist) {
      strncat(buffer, format_l_line(), sizeof(buffer) - 1);
      strncat(buffer, format_s_header(), sizeof(buffer) - 1);
...
int
do_operations_status(dbref enactor)
{
  static char buffer[BUFFER_LEN];
  ...
  if (error_on_console(enactor)) {
    ...
    strncat(buffer, "\n", sizeof(buffer) - 1);
    if (sdb[n].structure.type < 3 && (sdb[n].structure.has_docking_bay ||
                                      sdb[n].structure.has_landing_pad)) {
...
int
do_allocation_status(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    strncat(buffer,
            tprintf("      %s%-16.16s%s %10.10s: %4.4s %s\n", ANSI_CYAN, "ECM",
                    ...
                    ansi_rainbow_scale(sdb[n].alloc.ecm, 35)),
            sizeof(buffer) - 1);
    strncat(buffer,
            tprintf("      %s%-16.16s%s %10.10s: %4.4s %s\n", ANSI_CYAN, "ECCM",
                    ANSI_WHITE,
    ...
    strncat(buffer,
            tprintf("    %s%-18.18s%s %10.10s: %4.4s %s\n", ANSI_CYAN,
                    ...
                    ansi_rainbow_scale(sdb[n].alloc.tractors, 35)),
            sizeof(buffer) - 1);
    strncat(
      buffer,
      tprintf("    %s%-18.18s%s %10.10s: %4.4s %s\n", ANSI_CYAN,
              "Miscellaneous", ANSI_WHITE,
              unparse_power(sdb[n].alloc.miscellaneous * sdb[n].power.total),
              unparse_percent(sdb[n].alloc.miscellaneous),
              ansi_rainbow_scale(sdb[n].alloc.miscellaneous, 35)),
      sizeof(buffer) - 1);
    strncat(buffer, format_l_end(), sizeof(buffer) - 1);
...
int
do_damage_status(int type, char *name, dbref enactor)
{
  static char buffer[BUFFER_LEN * 2];
  register int i;
  int x;
  ...
  strncat(
    buffer,
    tprintf("%sSystem Name         Integ  Condition%s\n%s------------------ "
            "------- ----------------------------------------------------%s\n",
            ANSI_CYAN, ANSI_WHITE, ANSI_BLUE, ANSI_WHITE),
    sizeof(buffer) - 1);
  if (sdb[x].structure.superstructure != sdb[x].structure.max_structure)
    strncat(buffer,
            tprintf("%s%-18.18s%s %7.7s %-15.15s %10.3f\n", ANSI_CYAN,
  ...
  if (sdb[x].engine.impulse_exist && sdb[x].engine.impulse_damage < 1.0)
    strncat(buffer,
            tprintf("%s%-18.18s%s %7.7s %-15.15s %10.3f\n", ANSI_CYAN,
                    ...
                    (1.0 - sdb[x].engine.impulse_damage) * repair_mult[6] *
                      (1.0 + (sdb[x].structure.max_structure / 10.0)) * 100.0),
            sizeof(buffer) - 1);
  ...
  if (sdb[x].missile.exist)
    for (i = 0; i < sdb[x].missile.tubes; ++i)
      if (sdb[x].mlist.damage[i] < 1.0)
        strncat(buffer,
                tprintf("%s%-15.15s %2d%s %7.7s %-15.15s %10.3f\n", ANSI_CYAN,
                        system_name[9], i + 1, ANSI_WHITE,
                        unparse_percent(sdb[x].mlist.damage[i]),
                        unparse_damage(sdb[x].mlist.damage[i]),
                        (1.0 - sdb[x].mlist.damage[i]) * repair_mult[9] *
                          sdb[x].mlist.warhead[i] * 100.0),
                sizeof(buffer) - 1);
  if (sdb[x].shield.exist)
    for (i = 0; i < MAX_SHIELD_NAME; ++i)
      if (sdb[x].shield.damage[i] < 1.0)
        strncat(buffer,
                tprintf("%s%-18.18s%s %7.7s %-15.15s %10.3f\n", ANSI_CYAN,
                        ...
                        (1.0 - sdb[x].shield.damage[i]) * repair_mult[10] *
                          (1.0 + (sdb[x].structure.max_structure / 10.0)) *
                          100.0),
                sizeof(buffer) - 1);
  ...
  if (sdb[x].engine.warp_exist && sdb[x].engine.warp_damage < 1.0)
    strncat(buffer,
            tprintf("%s%-18.18s%s %7.7s %-15.15s %10.3f\n", ANSI_CYAN,
                    ...
                    (1.0 - sdb[x].engine.warp_damage) * repair_mult[14] *
                      (1.0 + (sdb[x].structure.max_structure / 10.0)) * 100.0),
            sizeof(buffer) - 1);
  strncat(buffer, format_l_line(), sizeof(buffer) - 1);
  strncat(
    ...
    tprintf(
      ...
      unparse_percent_3(sdb[n].structure.repair / sdb[n].structure.max_repair)),
    sizeof(buffer) - 1);
  strncat(buffer, format_l_end(), sizeof(buffer) - 1);
...
int
do_science_status(dbref enactor)
{
  static char buffer[BUFFER_LEN];
...
int
do_fighter_status(dbref enactor)
{
  static char buffer[BUFFER_LEN];
...
int
do_scanner_report(int a, char *s, dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    case 1:
      if (sdb[x].beam.banks) {
        ...
        strncat(buffer, ansi_red("No beam weapons detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
    case 2:
      if (sdb[x].cloak.exist || sdb[x].shield.exist) {
        ...
        strncat(buffer, ansi_red("No defensive systems detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
    case 3:
      if (sdb[x].main.exist || sdb[x].aux.exist || sdb[x].batt.exist ||
          sdb[x].engine.warp_exist || sdb[x].engine.impulse_exist) {
        strncat(buffer, format_s_header(), sizeof(buffer) - 1);
        if (sdb[x].main.exist)
        ...
        strncat(buffer, ansi_red("No engine systems detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
    case 4:
      if (sdb[x].main.exist || sdb[x].aux.exist || sdb[x].batt.exist) {
        ...
        strncat(buffer, ansi_red("No fuel systems detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
    ...
    case 6:
      if (sdb[x].missile.tubes) {
        ...
        strncat(buffer, ansi_red("No missile weapons detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
    case 7:
      if (sdb[x].trans.exist || sdb[x].tract.exist || sdb[x].sensor.lrs_exist ||
        ...
        strncat(buffer, ansi_red("No other systems detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
    ...
    case 9:
      ...
      if (d) {
        ...
        *dp = '\0';
        ...
        strncat(buffer, ansi_red("No visible features detected."),
                sizeof(buffer) - 1);
        strncat(buffer, "\n", sizeof(buffer) - 1);
      }
      break;
...
int
do_planet_report(const char *s, int t, dbref enactor)
{
  static char buffer[BUFFER_LEN];
  ...
  if (error_on_console(enactor)) {
    ...
    if (t) {
      for (x = MIN_SPACE_OBJECTS; x <= max_space_objects; ++x) {
        if (c < max) {
          if (sdb[n].space == sdb[x].space && sdb[x].structure.type == 5 &&
              n != x && SpaceObj(sdb[x].object) && GoodObject(sdb[x].object)) {
    ...
    if (c) {
      ...
      for (i = 0; i < max; ++i) {
        if (pln[i])
          strncat(buffer,
                  tprintf("%s%-20.20s%s %2.1s %7.7s %3.0f %-3.0f %7.7s %10.3f "
                          "%10.3f %10.3f\n",
                          ANSI_HILITE, Name(sdb[pln[i]].object), ANSI_NORMAL,
                          unparse_class(pln[i]), planet_eta(pln[i], rng[i]),
                          sdb2bearing(n, pln[i]), sdb2elevation(n, pln[i]),
                          unparse_range(rng[i]),
                          (sdb[pln[i]].coords.x - sdb[n].coords.xo) / PARSEC,
                          (sdb[pln[i]].coords.y - sdb[n].coords.yo) / PARSEC,
                          (sdb[pln[i]].coords.z - sdb[n].coords.zo) / PARSEC),
                  sizeof(buffer) - 1);
      }
      strncat(buffer, format_l_end(), sizeof(buffer) - 1);

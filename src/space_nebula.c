...
int
do_nebula_report(dbref enactor)
{
  ...
  if (error_on_console(enactor)) {
    ...
    snprintf(buffer, sizeof(buffer),
             "%s%s--[%sNebula "
             "Report%s]--------------------------------------------------------"
             "------%s\n%sNebulae              Bearing Range   Core Vis Core "
             "Coordinates%s\n%s-------------------- ------- ------- -------- "
             "---------------------------------%s\n",
      ...
      for (y = 0; y < 2; ++y) {
        for (x = 0; x < 2; ++x) {
          snprintf(nbuff, sizeof(nbuff), "Nebula %d-%d-%d",
                   round2(fabs(cx[x] / 100.0)), round2(fabs(cy[y] / 100.0)),
                   round2(fabs(cz[z] / 100.0)));
          dx = cx[x] * PARSEC;
          dy = cy[y] * PARSEC;
          dz = cz[z] * PARSEC;
          strncat(
            ...
            tprintf(
              ...
              (int) round2(
                xy2bearing((dx - sdb[n].coords.x), (dy - sdb[n].coords.y))),
              (int) round2(xyz2elevation((dx - sdb[n].coords.x),
                                         (dy - sdb[n].coords.y),
                                         (dz - sdb[n].coords.z))),
              unparse_range(xyz2range(sdb[n].coords.x, sdb[n].coords.y,
                                      sdb[n].coords.z, dx, dy, dz)),
              xyz2vis(dx, dy, dz) * 100.0, cx[x] - sdb[n].coords.xo / PARSEC,
              cy[y] - sdb[n].coords.yo / PARSEC,
              cz[z] - sdb[n].coords.zo / PARSEC),
            sizeof(buffer) - 1);
        }
      }
    }

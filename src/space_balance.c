...
void
balance_eng_power(void) /* Balances the eng alloc */
{
  double total =
    sdb[n].alloc.helm + sdb[n].alloc.tactical + sdb[n].alloc.operations;
...
void
balance_helm_power(void) /* Balances the helm alloc */
{
  double total;
...
void
balance_shield_power(void) /* Balances the shield alloc */
{
  register int i;
  double total = 0.0;
...
void
balance_tact_power(void) /* Balances the tactical alloc */
{
  double total;
...
void
balance_sensor_power(void) /* Balances the sensor alloc */
{
  double total;
...
void
balance_ops_power(void) /* Balances the operations alloc */
{
  double total;

  if (sdb[n].alloc.operations > 0.0) {
    total = sdb[n].alloc.transporters + sdb[n].alloc.tractors +

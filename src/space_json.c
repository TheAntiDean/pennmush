...
space_json_iterate()
{
  ...
  get_space_status("senrep",NULL, x, show_message, &smp);
		...
		while (pq) {
			...
			{			
				...
				if (b != NULL) {
					...
					if (GoodObject(user) && has_flag_by_name(user, "SPACE-JSON", TYPE_PLAYER)) {
						notify(user, tprintf("<SENREP>%s</SENREP>", show_message));
					}
				}
			}
		}
  ...
  cJSON *get_sensor_report(int shipSDB)
  {
    ...
    if (contacts) {
      ...
      for (contID = 0; contID < sdb[shipSDB].sensor.contacts; ++contID) {
        ...
        cJSON_AddNumberToObject(contact, "HeadingX",
                                sdb[contSDB].course.yaw_out);
  ...
  cJSON *get_ship_power(int shipSDB)
  {
    ...
    if (power) {
      ...
      cJSON_AddNumberToObject(engPower, "Tactical",
                              sdb[shipSDB].alloc.tactical);
      ...
      if (helmPower) {
        ...
        if (sdb[shipSDB].shield.exist) {
          ...
          if (shields) {
            cJSON_AddNumberToObject(shields, "Power",
                                    sdb[shipSDB].alloc.shields);
            int iterS = 0;
      ...
      if (tactPower) {
        ...
        cJSON_AddNumberToObject(tactPower, "Sensors",
                                sdb[shipSDB].alloc.sensors);
        cJSON *sensors = NULL;

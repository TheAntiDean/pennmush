...
quick_space_unparse(dbref object)
{
  ...
  case HOME:
    return "*HOME*\0";
  default:
    return tprintf("%s(#%d%s)", Name(object), object,
...
void
write_logfile(char *timestring, char *unp1, char *unp2, char *message)
{
  FILE *fp = fopen(aspace_log, "a");
...
void
write_logchannel(char *timestring, char *unp1, char *unp2, char *message)
{
  CHAN *c = NULL;
  ...
  if (find_channel(LOG_CHANNEL, &c, GOD) == CMATCH_NONE) {
    ...
    do_cemit(GOD, LOG_CHANNEL,
             tprintf("%s SPACE: %s %s %s", timestring, unp1, unp2, message), 0);
  }
}

void
write_spacelog(dbref executor, dbref object, char *fmt)
{
  struct tm *ttm;
  char timebuf[18];
  ...
  switch (LOG_TYPE) {
  case 1:
    write_logfile(timebuf, unp1, unp2, message);
    break;
  case 2:

...
{
  if (sdb[n].main.exist) {
    double dmg = (sdb[n].main.out - sdb[n].main.damage) * sdb[n].move.dt /
                 sdb[n].tech.main_max / 1000.0;
    ...
    if (sdb[n].main.damage <= -1.0) {
      ...
      do_all_console_notify(
        n, tprintf(ansi_warn("%s core breach."), system_name[8]));
...
{
  if (sdb[n].aux.exist) {
    double dmg = (sdb[n].aux.out - sdb[n].aux.damage) * sdb[n].move.dt /
                 sdb[n].tech.aux_max / 1000.0;
    ...
    if (sdb[n].aux.damage <= -1.0) {
      ...
      do_all_console_notify(
        n, tprintf(ansi_warn("%s core breach."), system_name[1]));
...
up_fuel(void)
{
  double mloss = sdb[n].main.out * sdb[n].main.out * sdb[n].main.gw * 100.0 /
                 sdb[n].tech.fuel * sdb[n].move.dt;
  double aloss = sdb[n].aux.out * sdb[n].aux.out * sdb[n].aux.gw * 100.0 /
                 sdb[n].tech.fuel * sdb[n].move.dt;
  ...
  if (sdb[n].fuel.antimatter < 0.0) {
    if (sdb[n].main.out > 0.0)
      alert_anti_runout(n);
    sdb[n].fuel.antimatter = sdb[n].main.in = sdb[n].main.out =
      sdb[n].power.main = 0.0;
    sdb[n].power.version = 1;
  }
  if (sdb[n].fuel.deuterium < 0.0) {
    if (sdb[n].aux.out > 0.0 || sdb[n].main.out > 0.0)
      alert_deut_runout(n);
    sdb[n].fuel.deuterium = sdb[n].main.in = sdb[n].main.out =
      sdb[n].power.main = sdb[n].aux.in = sdb[n].aux.out = sdb[n].power.aux =
        0.0;
    sdb[n].power.version = 1;
  }
  return;
...
{
  sdb[n].fuel.reserves +=
    (((sdb[n].power.main + sdb[n].power.aux + sdb[n].power.batt) *
      sdb[n].alloc.miscellaneous) -
     sdb[n].power.batt) *
    sdb[n].move.dt;
  if (sdb[n].fuel.reserves < 0.0) {
    sdb[n].fuel.reserves = sdb[n].batt.in = sdb[n].batt.out =
  ...
  if (sdb[n].engine.warp_exist)
    if (fabs(sdb[n].move.out) >= 1.0)
      if (fabs(sdb[n].move.out) > sdb[n].engine.warp_cruise) {
        sdb[n].engine.warp_damage -=
          (fabs(sdb[n].move.out) - sdb[n].engine.warp_cruise) * sdb[n].move.dt /
          sdb[n].tech.main_max / 10000.0;
...
void
up_impulse_damage(void)
{
  if (sdb[n].engine.impulse_exist)
    if (fabs(sdb[n].move.out) < 1.0)
      if (fabs(sdb[n].move.out) > sdb[n].engine.impulse_cruise &&
          fabs(sdb[n].move.in) < 1.0) {
        sdb[n].engine.impulse_damage -=
          (fabs(sdb[n].move.out) - sdb[n].engine.impulse_cruise) *
          sdb[n].move.dt / sdb[n].tech.aux_max / 10000.0;
...
up_impulse_max(void)
{
  ...
  if ((sdb[n].move.in > 0.0) && (sdb[n].move.in < 1.0) &&
      (sdb[n].move.in > sdb[n].engine.impulse_max)) {
    sdb[n].move.in = sdb[n].engine.impulse_max;
  } else if ((sdb[n].move.in > -1.0) && (sdb[n].move.in < 0.0) &&
...
up_tract_status(void)
{
  ...
  if (sdb[n].status.tractoring) {
    ...
    p = sdb[n].tract.damage * sdb[n].power.total * sdb[n].alloc.tractors /
        (sdb2range(n, x) + 1.0);
    if ((sdb[x].tract.active && p < sdb[x].tract.damage * sdb[x].power.total *
    ...
    p = sdb[x].tract.damage * sdb[x].power.total * sdb[x].alloc.tractors /
        (sdb2range(x, n) + 1.0);
    if (p <
        (sdb[n].tract.damage * sdb[n].power.total * sdb[n].alloc.tractors)) {
...
up_missile_io(void)
{
  if (sdb[n].missile.out > sdb[n].missile.in) {
    ...
    sdb[n].missile.out +=
      sdb[n].alloc.missiles * sdb[n].power.total * sdb[n].move.dt;
    if (sdb[n].missile.out >= sdb[n].missile.in) {
      sdb[n].missile.out = sdb[n].missile.in;
...
up_speed_io(void)
{
  ...
  if (fabs(sdb[n].move.out) < 1.0) {
    if (fabs(sdb[n].move.in) >= 1.0) {
      a = sdb[n].power.main * 0.99 +
          sdb[n].power.total * sdb[n].alloc.movement * 0.01;
    } else
      a = sdb[n].power.aux * 0.9 +
          sdb[n].power.total * sdb[n].alloc.movement * 0.1;
    a *= (1.0 - fabs(sdb[n].move.out)) / sdb[n].move.ratio / 50.0;
  } else
    a = (sdb[n].power.main * 0.99 +
         sdb[n].power.total * sdb[n].alloc.movement * 0.0) /
        sdb[n].move.ratio / fabs(sdb[n].move.out) / 5.0;
...
up_turn_rate(void)
{
  ...
  if (fabs(sdb[n].move.out) < 1.0) {
    ...
    a = 3.6 *
        (sdb[n].power.main * 0.99 +
         sdb[n].power.total * sdb[n].alloc.movement * 0.0) /
        sdb[n].move.ratio / fabs(sdb[n].move.out);
...
up_visibility(void)
{
  if (sdb[n].status.docked || sdb[n].status.landed) {
    ...
    sdb[n].sensor.visibility =
      xyz2vis(sdb[n].coords.x, sdb[n].coords.y, sdb[n].coords.z);
  }
  return;
}
...
up_resolution(void)
{
  ...
  if (sdb[n].sensor.lrs_active) {
    sdb[n].sensor.lrs_resolution =
      sdb[n].tech.sensors * sdb[n].sensor.lrs_damage;
  ...
  if (sdb[n].sensor.srs_active) {
    sdb[n].sensor.srs_resolution =
      sdb[n].tech.sensors * sdb[n].sensor.srs_damage;
    if (sdb[n].sensor.ew_active)
...
{
  double base =
    pow(sdb[x].structure.displacement, 0.333333) / sdb[x].tech.stealth / 100.0;
  double sig = base;
  // double cloak;
...
void
up_sensor_message(int contacts)
{
  ...
  for (i = 0; i < contacts; ++i) {
    ...
    if (!gain) {
      ...
      console_message(
        ...
        ansi_warn(tprintf("New sensor contact (%d): %s", temp_num[i],
                          unparse_type(temp_sdb[i]))));
    }
  }
    ...
    if (!lose) {
      ...
      if (sdb[n].trans.s_lock == sdb[n].slist.sdb[i]) {
        console_message(
          ...
          ansi_warn(tprintf("%s source lock on %s lost", system_name[13],
                            unparse_identity(n, sdb[n].slist.sdb[i]))));
      ...
      if (sdb[n].trans.d_lock == sdb[n].slist.sdb[i]) {
        console_message(
          n, "operation transporter",
          ansi_warn(tprintf("%s lock on %s lost", system_name[13],
                            unparse_identity(n, sdb[n].slist.sdb[i]))));
      ...
      if (sdb[n].tract.lock == sdb[n].slist.sdb[i]) {
        console_message(
          n, "helm operation",
          ansi_warn(tprintf("%s lock on %s lost", system_name[12],
                            unparse_identity(n, sdb[n].slist.sdb[i]))));
        sdb[n].tract.lock = 0;
        sdb[n].status.tractoring = 0;
      ...
      if (flag)
        console_message(
          n, "tactical",
          ansi_warn(tprintf("%s lock on %s lost", system_name[3],
                            unparse_identity(n, sdb[n].slist.sdb[i]))));
      flag = 0;
      ...
      if (flag)
        console_message(
          n, "tactical",
          ansi_warn(tprintf("%s lock on %s lost", system_name[9],
                            unparse_identity(n, sdb[n].slist.sdb[i]))));
    }
  }
...
up_sensor_list(void)
{
  ...
  for (object = MIN_SPACE_OBJECTS; object <= max_space_objects; ++object)
    if (sdb[n].location == sdb[object].location &&
        sdb[n].space == sdb[object].space && sdb[object].structure.type &&
        n != object) {
      ...
      if (z > limit)
        continue;
      level = (sdb[n].sensor.srs_resolution + 0.01) *
              sdb[object].sensor.srs_signature /
              (0.1 + (x * x + y * y + z * z) / 10101.010101);
      x /= PARSEC;
      y /= PARSEC;
      z /= PARSEC;
      level += sdb[n].sensor.lrs_resolution * sdb[object].sensor.lrs_signature /
               (1.0 + (x * x + y * y + z * z) * 99.0);
...
up_repair(void)
{
  sdb[n].structure.repair +=
    sdb[n].move.dt * sdb[n].structure.max_repair / 1000.0 *

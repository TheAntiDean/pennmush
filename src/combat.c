...
COMMAND(cmd_local_attack)
{
  ...
  if (aStats.Stamina < 1) {
    notify_format(executor, "%sGAME: %s%sYou have no stamina%s", ANSI_HIGREEN, ANSI_END, ANSI_CYAN, ANSI_ENDALL);
    return;
  }
  ...
  if (aStats.AttackSkill > 99) {
    aStats.AttackSkill = 99;
  } else if (aStats.AttackSkill < 1) {
    aStats.AttackSkill = 1;
  }
  ...
  if (rand() % 99 < aStats.AttackSkill) {
    // We find out if the defender dodges
    if (tStats.DodgeSkill > 99) {
      tStats.DodgeSkill = 99;
    } else if (tStats.DodgeSkill < 1) {
      tStats.DodgeSkill = 1;
    }

    if (rand() % 99 < tStats.DodgeSkill && tStats.Stamina > 0) {
      do_defend(executor, target);
      safe_format(
        msg, &bp, "%sGAME:%s %s%s%s%s attacks %s%s%s%s but they dodge.%s",
        ANSI_HIGREEN, ANSI_END, ANSI_HIWHITE, Name(executor), ANSI_END,
        ANSI_CYAN, ANSI_HIWHITE, Name(target), ANSI_END, ANSI_CYAN, ANSI_ENDALL);
      notify_except(executor, Location(executor), executor, msg, 0);
      notify_format(executor, "%sGAME: %s%sYou attack %s%s%s but they dodge.%s",
                    ANSI_HIGREEN, ANSI_END, ANSI_CYAN, ANSI_HIWHITE,
                    Name(target), ANSI_END, ANSI_CYAN, ANSI_ENDALL);
      
    } else {

        do_attack(executor, target);
              safe_format(
        msg, &bp, "%sGAME:%s %s%s%s%s attacks %s%s%s%s and hits.%s",
        ANSI_HIGREEN, ANSI_END, ANSI_HIWHITE, Name(executor), ANSI_ENDALL,
        ANSI_CYAN, ANSI_HIWHITE, Name(target), ANSI_END, ANSI_CYAN, ANSI_ENDALL);
      notify_except(executor, Location(executor), executor, msg, 0);
      notify_format(executor, "%sGAME: %s%sYou attack %s%s%s and hit.%s",
                    ANSI_HIGREEN, ANSI_END, ANSI_CYAN, ANSI_HIWHITE,
                    Name(target), ANSI_END, ANSI_CYAN, ANSI_ENDALL);
    }
    ...
    safe_format(msg, &bp, "%sGAME:%s %s%s%s%s attacks %s%s%s%s but misses.%s",
                ANSI_HIGREEN, ANSI_END, ANSI_HIWHITE, Name(executor), ANSI_ENDALL,
                ANSI_CYAN, ANSI_HIWHITE, Name(target), ANSI_END, ANSI_CYAN,
                ANSI_END);
    notify_except(executor, Location(executor), executor, msg, 0);
    notify_format(executor, "%sGAME: %s%sYou attack %s%s%s but you miss.%s",
                  ANSI_HIGREEN, ANSI_END, ANSI_CYAN, ANSI_HIWHITE, Name(target),
  ...
  *bp = '\0';
  ...
  /*
   ...
   - Determine Defender dbref - Done
     ...
     - Check his contents for equipped armor/weapons and increment/decrement his
  ...
  ATTACK:
    - pick a random number between 0 and 100 for the attacker's roll. - done
    - Get the attacker's attack skill - done
      - add in any attack bonuses and subtract any attack penalties provided by
  weapons/armor, but limit between 1 and 99. - done
  ...
  DEFEND:
    ...
    - Get the defender's dodge skill
      - add in any dodge bonuses and subtract any dodge penalties provided by
  weapons/armor, but limit between 1 and 99.
...
setupCombatAttr()
{
  ...
  add_new_attr(
    "Damage", AF_WIZARD); // How much damage the weapon (or player) can dish out
...
getStats(dbref obj)
{
  ...
  cstat.Health = parse_number(getAtrValue(obj, "HEALTH"));
  cstat.MaxHealth = parse_number(getAtrValue(obj, "MAXHEALTH"));
  cstat.Stamina = parse_number(getAtrValue(obj, "STAMINA"));
  cstat.MaxStamina = parse_number(getAtrValue(obj, "MAXSTAMINA"));
  cstat.AttackSkill = parse_number(getAtrValue(obj, "SKILL`ATTACK"));
  cstat.DodgeSkill = parse_number(getAtrValue(obj, "SKILL`DODGE"));
  cstat.EquippedBy = parse_dbref(getAtrValue(obj, "EQUIPPEDBY"));
  cstat.Damage = parse_number(getAtrValue(obj, "DAMAGE"));
...
{
  return (has_flag_by_name(obj, "ARMOR", NOTYPE) ||
          has_flag_by_name(obj, "WEAPON", NOTYPE)) &&
         IsThing(obj);
}

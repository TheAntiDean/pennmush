...
void
damage_structure(int x, double damage)
{
  register int i;
  ...
  console_message(
    x, "damage engineering",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[0], ANSI_WHITE,
  ...
  if ((sdb[x].structure.superstructure <= -sdb[x].structure.max_structure) &&
    ...
    do_ship_notify(x, tprintf("%s%s%s%s explodes into white hot vapor.%s%s",
                              ANSI_HILITE, ANSI_INVERSE, ANSI_RED,
                              Name(sdb[x].object), ANSI_WHITE, ANSI_NORMAL));
    /* End Aspace v1.0.0p1*/
    do_space_notify_one(x, console_helm, console_tactical, console_science,
                        "has been destroyed");
    write_spacelog(
      sdb[x].object, sdb[x].object,
      tprintf("LOG: Destroyed, Shields %.6f GHz", sdb[x].shield.freq));
    sdb[x].space = -1;
    ...
    for (i = MIN_SPACE_OBJECTS; i <= max_space_objects; ++i)
      if (sdb[i].location == x)
        if (sdb[i].structure.type) {
          do_ship_notify(
            i, tprintf("%s%s%s%s explodes into white hot vapor. Goodbye!%s%s",
                       ANSI_HILITE, ANSI_INVERSE, ANSI_RED, Name(sdb[i].object),
    ...
    do_all_console_notify(
      x, ansi_warn("Excessive damage. All systems shutting down"));
    do_ship_notify(
      x, tprintf("%s experiences total systems failure.", Name(sdb[x].object)));
    sdb[x].status.crippled = 1;
    do_space_notify_one(x, console_helm, console_tactical, console_science,
                        "has been disabled");
    write_spacelog(
      sdb[x].object, sdb[x].object,
      tprintf("LOG: Disabled, Shields %.6f GHz", sdb[x].shield.freq));
  }
...
void
damage_aux(int x, double damage)
{
  if (!sdb[x].aux.exist || sdb[x].aux.damage == -1.0)
    return;
  if (sdb[x].power.aux != 0.0 && sdb[x].aux.damage > 0.0 &&
  ...
  if (sdb[x].aux.damage < -1.0) {
    ...
    if (sdb[x].power.aux != 0.0) {
      do_all_console_notify(
        x, tprintf(ansi_warn("%s core breach."), system_name[1]));
      damage_structure(x, sdb[x].power.aux * (get_random_long(0, 10) + 1.0));
    }
  }
  console_message(
    x, "damage engineering",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[1], ANSI_WHITE,
...
void
damage_batt(int x, double damage)
{
  ...
  console_message(
    x, "damage engineering",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[2], ANSI_WHITE,
...
void
damage_beam(int x, int beam, double damage)
{
  if (!sdb[x].beam.exist || sdb[x].blist.damage[beam] == -1.0)
    return;
  sdb[x].blist.damage[beam] -=
    damage / ((sdb[x].blist.cost[beam] + sdb[x].blist.bonus[beam]) / 10.0);
  if (sdb[x].blist.damage[beam] < -1.0)
    sdb[x].blist.damage[beam] = -1.0;
  console_message(
    x, "damage tactical",
    ansi_alert(tprintf("%s%s %d%s: %s %s", ANSI_CYAN, system_name[3], beam + 1,
                       ANSI_WHITE, unparse_percent(sdb[x].blist.damage[beam]),
...
void
damage_cloak(int x, double damage)
{
  if (!sdb[x].cloak.exist || sdb[x].cloak.damage == -1.0)
    return;
  sdb[x].cloak.damage -=
    damage / (1.0 + (sdb[x].structure.max_structure / 100.0));
  if (sdb[x].cloak.damage < -1.0)
    sdb[x].cloak.damage = -1.0;
  console_message(
    x, "damage helm",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[4], ANSI_WHITE,
...
void
damage_ew(int x, double damage)
{
  if (!sdb[x].sensor.ew_exist || sdb[x].sensor.ew_damage == -1.0)
    return;
  sdb[x].sensor.ew_damage -=
    damage / (1.0 + (sdb[x].structure.max_structure / 10.0));
  if (sdb[x].sensor.ew_damage < -1.0)
    sdb[x].sensor.ew_damage = -1.0;
  console_message(
    x, "damage science tactical",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[5], ANSI_WHITE,
...
void
damage_impulse(int x, double damage)
{
  ...
  console_message(
    x, "damage engineering",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[6], ANSI_WHITE,
...
void
damage_lrs(int x, double damage)
{
  ...
  console_message(
    x, "damage science tactical",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[7], ANSI_WHITE,
...
void
damage_main(int x, double damage)
{
  if (!sdb[x].main.exist || sdb[x].main.damage == -1.0)
    return;
  if (sdb[x].power.main != 0.0 && sdb[x].main.damage > 0.0 &&
  ...
  if (sdb[x].main.damage < -1.0) {
    ...
    if (sdb[x].power.main != 0.0) {
      do_all_console_notify(
        x, tprintf(ansi_warn("%s core breach."), system_name[8]));
      damage_structure(x, sdb[x].power.main * (get_random_long(0, 100) + 1.0));
    }
  }
  console_message(
    x, "damage engineering",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[8], ANSI_WHITE,
...
void
damage_missile(int x, int missile, double damage)
{
  if (!sdb[x].missile.exist || sdb[x].mlist.damage[missile] == -1.0)
    return;
  sdb[x].mlist.damage[missile] -=
    damage / (sdb[x].mlist.warhead[missile] / 10.0);
  if (sdb[x].mlist.damage[missile] < -1.0)
    sdb[x].mlist.damage[missile] = -1.0;
  console_message(x, "damage tactical",
                  ansi_alert(tprintf(
                    "%s%s %d%s: %s %s", ANSI_CYAN, system_name[9], missile + 1,
                    ANSI_WHITE, unparse_percent(sdb[x].mlist.damage[missile]),
...
void
damage_shield(int x, int shield, double damage)
{
  if (!sdb[x].shield.exist || sdb[x].shield.damage[shield] == -1.0)
    return;
  sdb[x].shield.damage[shield] -=
    damage / (1.0 + (sdb[x].structure.max_structure / 10.0));
  if (sdb[x].shield.damage[shield] < -1.0)
    sdb[x].shield.damage[shield] = -1.0;
  console_message(x, "damage helm",
                  ansi_alert(tprintf(
                    "%s%s%s: %s %s", ANSI_CYAN, unparse_shield(shield),
                    ANSI_WHITE, unparse_percent(sdb[x].shield.damage[shield]),
...
void
damage_srs(int x, double damage)
{
  ...
  console_message(
    x, "damage science tactical",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[11], ANSI_WHITE,
...
void
damage_tract(int x, double damage)
{
  ...
  console_message(
    x, "damage operation",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[12], ANSI_WHITE,
...
void
damage_trans(int x, double damage)
{
  ...
  console_message(
    x, "damage operation transporter",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[13], ANSI_WHITE,
...
void
damage_warp(int x, double damage)
{
  ...
  console_message(
    x, "damage engineering",
    ansi_alert(tprintf("%s%s%s: %s %s", ANSI_CYAN, system_name[14], ANSI_WHITE,
...
void
dump_space(dbref executor) /* dumps the sdb[] struct into the space objects */
{
...
debug_space(int x)
{
  ...
  /* --- SHIELD ------------------------------------------------------------- */
  if (!sdb[x].shield.exist || sdb[x].shield.ratio <= 0.0 ||
